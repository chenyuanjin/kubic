...
| [`U6.2` 屏与端的能力矩阵](U6.2-屏与端的能力矩阵.md) | 十四屏 × 四端（2026-09-03 加入 `S-ASK`），每一格取什么值 | 每一格只有三种取值；🔴 **每个缺失格子必须有降级路径**，没有降级路径的登记成缺口；三格之外没有第四格 |
...
| `M6-G8` | **明确转场之后目标端打不开（没装 / 没网），用户停在哪** | 真源没写。转场是唯一会把用户交给另一个端的动作，交出去之后没有落点等于把人堵在原地 | **产品**，未定。落点 `U6.2` §五 · §7.1 第 3 项 |
| `M6-G9` | **首页两个计数到底可不可点** | 两份规格结论相反：一份写成可点，无障碍真源的焦点全表里没有它们。可点就要进焦点链、要有可读的动作名，两件事一起变 | **产品裁定人**。落点 `U6.4` §五 · §6.1 ※8·※9 |

⚠️ **`M6-G8` / `M6-G9` 是 2026-09-03 补登记的**：这两条此前只以 ⚪ 出现在单元正文与流程图里，**没有进任何一张缺口表**，缺口扫描数不到。
