...
> 裁定的是**它放哪儿**,不是它属于哪条管线 —— 两者是两个维度(`文档规范与目录` §3.2)。
> `docs/misc/` 收的就是「三条管线的读者判据都套不上」的文档,**它不是待办清单,也不是垃圾桶**:
> 进 `misc/` 必须在 `文档规范与目录` §2.4 或 §2.3 有一条对应登记(`文档规范与目录` §3.2)。
> **管线归属另有已存在的裁定**:`总路线图 · 脑图与父子待办` `R-90` 末尾「2026-08-29 定:归入产品轨(线 1)」——
> 那一句在本轮之前从未同步进 `文档规范与目录` §2.1,它本身就是「同一件事两处说法」的一个实例,已一并改掉。
...
**一个能编排七个阶段却没人用的 agent,正是 `思考模式与选择框架` §盲区二 说的那种「能做的部分」。**
...
**依赖方向**:`kaodian-agent` 只依赖 `kaodian-domain`。它看不见 `auth`,也看不见 `app` —— 由 pom 强制,不靠自觉(`技术架构与接口契约` §2.3 加注)。

## 四、能力边界:三道防线,今天有两道 🔴

`后端系统设计与组件接入` §1.3 对打标链路的原话是:**「不是靠 prompt 里写一句『不要讲解』,是在输出侧检」**。
...
| 项 | 内容 |
|---|---|
| `R-90` | **阶段归属未定。** `实施路径`/`总路线图 · 脑图与父子待办` 的三条线里没有它的位置。它不是产品轨的阶段项,也不是合规轨或数据轨。**要么给它一个阶段,要么承认它是超出排期的一次探索** |
| `R-91` | 输出侧自检缺席(见 §四) |
| `R-92` | `/api/agent/chat` **无鉴权、无频控、无额度**,而它每次调用产生真实外部账单。`R-20`:「失控点是重复调用不是单价」。额度(`ai_ask`)标着阶段 2 后,所以「没额度」与阶段一致 —— **但「没额度」和「没频控」是两件事** |

> **自检:这一节比前六节重要。** 前六节描述的是一个已经写好的东西;这一节问的是它该不该在这个阶段存在。`总路线图 · 脑图与父子待办` §六 的判据没有变——**看 `1.1.4` 那张表有没有在填**。
