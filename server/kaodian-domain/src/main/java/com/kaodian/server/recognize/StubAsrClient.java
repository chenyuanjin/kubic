 ...
 * (01 §2.2 宁缺毋滥 / docs/09 坑一)。
 * <p>
 * docs/10 §3.1 对这个接口的注释是「失败抛异常,不返回半成品」。空串、占位符、
 * 「[识别失败]」这类值都是半成品 —— 它们会被下游当成真结果继续用。
 *
 * <h2>它失败了也不会让记录丢掉</h2>
 *
 * docs/08 §1.3.7.1:识别服务不可用时,<b>记录动作本身永不失败</b>。
 * 这个异常在 {@link com.kaodian.server.collect.CaptureService} 里被接住。
 *
 * <p>换成真实实现(腾讯云一句话识别,docs/09 §一)时,只需要多一个 {@link AsrClient}
 * 实现类并让它优先生效 —— 这正是 docs/09 坑三要的那个切换点。
 */
@Component
