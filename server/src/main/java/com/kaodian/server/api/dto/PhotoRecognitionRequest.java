 ...
 * 而 docs/10 §8.1 禁令 2 是「服务端不写磁盘、不进对象存储、不建图片桶」——
 ...
 * 那条红线在 docs/08 §四 上标着「<b>第一天不定就改不回来</b>」。
 ...
 * docs/10 §8.1 禁令 3 说的是「不把 base64 打进日志的任何级别」,而这一次打日志的是框架 ——
 ...
 *               单次 ≤ {@link #MAX_PHOTOS} 张 —— docs/10 §6.2「单次 ≤6 张(连拍合并,{@code 1.1.2.3})」
 */
public record PhotoRecognitionRequest(
    ...
    /**
     * 单次最多几张 —— docs/10 §6.2 逐字:「单次 ≤6 张(连拍合并,{@code 1.1.2.3})」。
     *
     * <p>{@code 1.1.2.3} 的场景是「听课连续截图」:6 张是<b>同一份材料的多张</b>,
     * 合并成<b>一条</b>记录,不是 6 条。上限的作用不只是省钱 ——
     * base64 内联下每多一张,请求体就多几百 KB,而 docs/10 §8.1 禁令 5 说得很清楚:
     ...
     * 也可能就是那张图的 base64。异常里只带字段名(01 §2.2 不碰内容 / docs/10 §8.1 禁令 3)。
     */
    @JsonAnySetter
