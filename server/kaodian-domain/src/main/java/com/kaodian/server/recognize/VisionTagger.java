 ...
 *
 * docs/识别链路 坑一(标注为「这条最重要」):<b>模型是分类器,不是标签生成器。</b>
 * 解法是「输入 = 图 + 自己的考点树候选,输出 = 树里的节点 ID 或『不匹配』」。
 * <p>
 * 这条约束在这里有三个抓手,少一个都能被绕过:
 * <ol>
 *   <li>{@code candidates} 是<b>必填入参</b> —— 不给候选集就没法调用,闭集不是可选项</li>
 *   <li>返回类型是 {@link RecognitionResult},里面<b>没有 String label 字段</b>(docs/技术架构 §3.1)</li>
 *   <li>{@link #enforceClosedSet} 在出口再核一遍:code 不在候选里就丢掉 ——
 *       docs/总路线图 {@code 1.2.5.1.6} 说得很清楚,<b>「不是靠 prompt 里写一句『不要讲解』,是在输出侧检」</b></li>
 * </ol>
 *
 * <h2>🔴 签名上没有 URL、没有 fileId、没有 bucket、没有任何存储路径</h2>
 *
 * docs/识别链路 坑二把厂商 Files API 列为红线项:它免费、看起来像白送的优化,
 ...
 * (docs/技术架构 §3.1)。签名里没有能装下「一个指向已存图片的引用」的位置,
 * 「传上去拿个 file_id 复用」这条路就不存在。
 * <p>
 * 实现类的三条附带禁令(docs/技术架构 §8.1):不写磁盘、不进对象存储、
...
public interface VisionTagger {
     ...
     * @param name 考点名。<b>自行归纳的命名</b>,不沿用机构既有体系与措辞(R-07 / docs/实施路径 §1.2)
     */
     ...
     *                   调用完即释放;不落盘、不进对象存储、不进日志(docs/技术架构 §8.1)
     ...
     *         <b>抛这个异常不等于记录失败</b> —— 见 docs/总路线图 §1.3.7.1
     */
     ...
     * (幻觉,或者干脆把机构的标准表述当 code 吐出来)。docs/识别链路 坑一说的
     * 「LLM 出错是<b>编造一个不存在的考点</b>」就是这个 —— 而错标会让覆盖度失真,
     * 覆盖度就是整个产品。
     *
     * <h2>为什么是丢弃而不是抛异常</h2>
     *
     * 模型胡说不该让用户的记录动作失败(docs/总路线图 §1.3.7.1)。按 决策记录 §2.2「宁缺毋滥」
