...
| 单元 | 管什么 | 关键决策 |
|---|---|---|
| [`U0.1` 首启产品说明屏](U0.1-首启产品说明屏.md) | 登录前那一屏说什么、能不能跳过 | 三句逐字同源于边界全文页，`scripts/boundary-copy-check.py` 守着；🔴 **是否作为不可跳过的首启第一屏，两份规格结论相反，登记待裁**（§七 `M0-G1`） |
...
| # | 缺口 | 缺的是哪一半 | 该谁定 |
|---|---|---|---|
| `M0-G1` | **首启要不要展示那三句，两份规格结论相反**：`首次使用与权限申请` §1.3.3 定成不可跳过的第一屏，`设置与原图` §3.3 判「不展示」，且两边互相都没改对方 | 缺的是一次裁定，不是缺信息 —— 两边的理由都完整，只是结论相反 | 🔴 **人裁定。** 原文与行号见 `U0.1` §五 |
