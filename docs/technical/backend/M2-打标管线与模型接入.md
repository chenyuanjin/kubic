...
四份模块设计各数出一个数（`B0` 三 / `M2` 四 / `M4` 五 / `M7` 五），stage 3 已取**并集八行**并落进 `接口契约` §3.3 —— **那张表是唯一真源，本文不再自己数**。`suggest` 是那八行里的第 5 行。
...
> **`POST /records/{id}/tags/suggest` 在 `接口契约` §3.3 那八行里（第 5 行）**，§4.1 正文已逐字写死它必带 `Idempotency-Key`。
