...
>
> 契约在 `docs/15-壳技术方案:Tauri 2 包现有 Web 工程.md`(`KUBI-62`),本文件只讲怎么跑、
> 以及代码里那几个「看起来像漏了」的地方为什么是有意的。**契约变更必须回去改那份文档** ——
> 反代那九条里任何一条被实现绕过,是文档过期,不是实现聪明。
>
> ⚠ **编号撞了,已在 `KUBI-65` 报出:** `v1` 上另有一份 `docs/15-Agent框架与能力边界.md`,
> 而 `KUBI-62` 分支上的壳技术方案也编到 15。本目录里所有 `docs/15 §x` 一律指**壳技术方案**那份。
> 两份合到同一条分支上时要重编一个号,那是 `KUBI-62` 的事,不由壳来改。
>
...
Developer ID + 公证,而本轮没有分发(`docs/15` §4.4)。
...
| 端口被占时直接不启动 | 换端口 = 换 origin = 浏览器侧本地存储静默全失(`R-73`) | `src/strings.rs` |
