...
**十三屏,一屏不多。** ⚠️ **2026-09-03：`S-ASK` 已作为第十四屏进入端矩阵**（`模块地图` §三登记的 `U3.7`、`U6.2` §2.2），**本表与下面这张屏幕表尚未跟上,也不由模块层改写** —— 缺口 `M3-G8` / `M6-G9`,该产品裁定。在它被改齐之前,「十三屏」这句话读作「十三屏 + 一屏有契约无规格的 `S-ASK`」。 屏幕编号 `S-XXX` 是设计与开发共用的锚,**改名可以,改编号要同时改本表与各规格文档**。
