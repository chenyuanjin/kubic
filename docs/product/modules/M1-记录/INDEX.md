...
| 单元 | 管什么 | 这一份文档独有的决策 |
|---|---|---|
| 🎯 **hub** · [`U1.1` 记录动作永不失败](U1.1-记录动作永不失败.md) | 整域命门，`I-1` 的落点。**本域另外六份都直接引用它**（`模块地图` §1.1 规则 2），不复制它的内容 | 允许失败的清单是**穷举**的；不在表上的都不许失败；14 个失败注入点各验哪一条 |
...
| `M1-G16` 「改一下再传」的入口位置 | 界面缺：补传被拒（4xx）那条卡片要给一条改完再传的路，真源没写明它落在哪一屏 —— 卡片内联 / 进详情 / 回 `S-REC`，三种落法对应三种记录归属 | 由人拍板。落点 `U1.6` §五 · §6.5 ※9 |

⚠️ **`M1-G16` 带域前缀**（`模块地图` §一 第 4 条：新号一律 `M<n>-G<k>`；存量 `G-1`–`G-15` 不改名，裸号只在本域有效）。
它是 2026-09-03 补登记的：此前只以一个没有编号的 ⚪ 出现在 `U1.6` §6.5 的注里，缺口扫描数不到。
