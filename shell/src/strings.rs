...
//! <h2>为什么集中</h2>
//!
//! docs/technical/壳技术方案-Tauri2包现有Web工程.md §六:「能力边界文案扫描」因此只有一处要扫。
...
/* ── 本机原图存储(KUBI-63 / R-105)────────────────────────────────────────── */
//
// 🔴 这三句【不上界面】。前端那一侧(`rawImageFs.ts`)把任何一种失败都翻成
// 同一句话:「这张图没被本地缓存」—— 而不是「记不下来」(docs/technical/后端系统设计与组件接入.md §1.5)。
// 这里的字符串只进错误体的 message 字段,给排查的人看。
//
// 三句里都【没有】id、没有文件名、没有路径:docs/technical/INDEX.md §8.2 明说路径也是设备信息,
// 而错误体是会穿过进程边界的东西。

/// 请求形状不对(缺元信息头、id 不在字符集里、body 不是预期的 JSON)。
pub const RAWIMAGE_BAD_REQUEST: &str = "本机原图存储没能理解这个请求。";

/// 磁盘那一侧出了问题(目录没权限、磁盘满、索引读不动)。
pub const RAWIMAGE_IO_FAILED: &str = "本机原图存储没能完成这次读写。";

/// 请求体超过上限。
pub const RAWIMAGE_TOO_LARGE: &str = "这次请求的内容超过了本机原图存储的上限。";

/// 索引里没有这一行,或者字节文件不在。
pub const RAWIMAGE_NOT_FOUND: &str = "本机原图存储里没有这一行。";
