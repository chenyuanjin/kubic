...
/// 双下划线开头是给读代码的人看的 —— **它不是产品 API**,不进 `docs/10` 的接口表。
/// 与 `web/src/lib/rawImageFs.ts` 里的 `BASE` 逐字相同,两侧靠 `docs/21 §9.1` 那张表对齐。
...
impl Server {
    ...
    /// 目录选在哪儿本身就是红线的物理落点,见 `raw_image_store.rs` 顶部与 `docs/21 §3.4`。
    ...
    /* ====================================================================== */
    /* 本机原图存储 —— 路由表见 docs/21 §9.1,web 侧 rawImageFs.ts 是同一张表      */
    /* ====================================================================== */
    ...
    /// docs/10 §8.2 明说路径也是设备信息,而这个 body 会走到前端。
