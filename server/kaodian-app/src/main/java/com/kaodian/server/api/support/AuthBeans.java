...
public class AuthBeans {
    ...
    /**
     * 微信。默认 {@link DisabledWeChatClient} —— <b>阶段 2 后才启用</b>(docs/technical/INDEX.md §7.2)。
     *
     * <p>三条入口是三个不同的应用,各要一笔认证费与一次审核。
     * 在阶段 2 之前掏这笔钱,买的是一个还没被验证的方向。
     */
    @Bean
     ...
     * 换一批 IP、换一批号两条都不触发,而每一条短信都是真金白银(docs/technical/后端系统设计与组件接入.md §1.8)。
     * <p>
    ...
    public VendorPairingCheck checkVendorPairing(SmsSender sender, CaptchaVerifier captcha) {
        if (sender.isReal() && !captcha.isReal()) {
            throw new IllegalStateException("""
                    ...
                    请配置 kaodian.auth.captcha.provider=tencent 及其四个配置项(docs/technical/后端系统设计与组件接入.md §1.8)。""");
        }
        if (!sender.isReal()) {
