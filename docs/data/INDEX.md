...
> 第三条线。与产品轨(关卡管辖)、合规轨(审批周期管辖)并列,由**风险边界**管辖。
>
...
前两条线的时钟分别是关卡和审批周期。这一条的时钟是**风险边界**:做对了它几乎零成本长期积累,做错了一次就能终结整个项目。
...
| 阶段 | 数据线做什么 |
|---|---|
| 阶段 0(第 1-2 周) | **什么都不做。** 关卡 0 都没过,项目可能整个不成立 |
| 关卡 0 通过后 | 开始 **A/B 级源清单调研**(纯整理,不写代码) |
| 关卡 1 通过后 | 启动自动化抓取,**只抓 A/B 级** |
| 关卡 2 通过后 | 评估是否扩到 C 级 |
...
| 时点 | 关卡 1 后持续后台 | **第一个模块跑通并验证之后** |
| 用户可见 | 否 | 是 |

**边界只有一句话:数据线可以为四个科目积累原料,但产品在关卡 2 之前只能有一个模块、一个科目。**
