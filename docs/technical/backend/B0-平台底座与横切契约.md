...
> **基线:`origin/v1` @ `81a2c23`,fetch 于 2026-09-03 14:31 CST。** 代码侧取证全部实读该基线,行号即该基线的行号。
>
...
| `B0-4` | 鉴权默认值 | **一个过滤器 + 一张写死的五行白名单常量**,白名单外无令牌一律 `401` | 一个测试枚举全部端点,断言白名单外的每一个无令牌时都 `401`(§五) |
| `B0-5` | 响应包络与错误码 | `code` 落成**枚举** `ErrorCode`,HTTP 状态挂在枚举上。`ApiError` 补第四个可选字段 `details` | 测试从 `接口契约` §十 抽出 **80 个码**,与 `ErrorCode.values()` 双向比对(§六) |
| `B0-6` | 分页与幂等 | 分页只有游标一种,形状 `{items, nextCursor?}`;幂等三种键各有其名,第三处是 `POST /ai/ask` **不是退款** | DTO 里 `total`/`hasMore`/`page`/`offset` 零命中 —— **今天不是零**(§七) |
...
🔴 **十条里没有一条是「待定」。** 需要人拍板的两处写在 §16.1,指了名、指了时点,且都不拦 stage 2 开工。
...
public class ApiAuthFilter extends OncePerRequestFilter {
    ...
    /** 🔴 匿名端点全集。加一行要在 接口契约 §三 同时加一行,两处不一致由 §5.5 判据 ② 判红。 */
...
| `/billing/notify/wxpay` | 🔴 **不是匿名,是另一条鉴权链** —— 独立验签,过滤器放行后由该 controller 自己验签。放进这张表是为了让「匿名的只有四个」不被误读成「回调漏了」 |
| `CurrentSessionResolver` | **保留**。过滤器负责「能不能进」,解析器负责「进来的是谁」。两件事,不合并 |

### 5.3 🔴 一处必须点破:`401` 要拆两档,而现在的代码刻意不拆

**`接口契约` §1.2 要求:**「`401` 必须能区分『从没登录』与『登录过期』:`UNAUTHORIZED` / `TOKEN_EXPIRED`。`U5.1` 要求这两档 —— 区分不出来登录门的副标题就写不出来。」
...
| 情形 | `code` | 泄露了什么 |
|---|---|---|
| 令牌存在、**仅仅是过期** | `TOKEN_EXPIRED` | **什么都没有** —— 持有这个令牌的人本来就知道它曾经有效 |
| 没带头 / 格式不对 / 查不到 / **已吊销** | `UNAUTHORIZED` | 已吊销与查不到仍然分不开,解析器注释那条顾虑原样保住 |
...
// ② 白名单与契约同源 —— 代码里五行,接口契约 §三 的表也必须是同五行
...
| ⚠️ **更正** | **实际是八处,不是三处** | 见下 | KUBI-96 stage 3 冲突裁定 1 |

⚠️ **2026-09-03(KUBI-96 stage 3):上面这个「三处」数漏了五处,已更正为八处。**
...
🔴 **八行的全集、各自的保留期与判据,以 [`接口契约:签名与错误码全集`](../接口契约-签名与错误码全集.md) §3.3 为唯一真源**,本节不再复述(复述一份就是第二真源)。
...
| 哪些端点要带 | 🔴 **`接口契约:签名与错误码全集` §3.3 那张八行表,唯一真源**(KUBI-96 已合入;本文 §十五 增量 2 是它的来源原文,保留作历史记录,**不要再合第二遍**) |
...
| 2 | **`401` 拆不拆档** | `接口契约` §1.2:必须区分 `UNAUTHORIZED` / `TOKEN_EXPIRED`;`CurrentSessionResolver.java` 注释:四种失败对外同一个 401,「对攻击者却是信息」 | **契约赢,但只拆两档**(§5.3)。「已吊销」不单独成档,解析器那条顾虑原样保住 |
...
> **这张表补第四列 `幂等`,取值 `—` / `Idempotency-Key`。** 今天要标 `Idempotency-Key` 的只有三行:`POST /billing/orders`(外部账单)、`POST /ai/ask`(🔴 外部模型调用 = 按次外部账单,重放一次扣两次额度)、`DELETE /account`(不可逆改变账号状态)。
>
> §1.5 写着「哪些端点必须带 `Idempotency-Key`,在 §三 的总表里逐行标 `幂等` 列」,而那一列此前不存在 —— 一条指向空处的指针,读的人会以为自己看漏了。

### 增量 3 · §8.8 退款那一节,补一句幂等的话

> ⚠️ **顺带关掉一处会被反复问起的联想:退款既然一个端点都不补,它就没有幂等语义。** §1.5 的三种幂等键里,「请求键」的三个落点是下单、`POST /ai/ask`、注销,**不含退款**。
...
### 增量 6 · §1.2 鉴权,补 `401` 两档的落法

> **`401` 两档的落法(与 `CurrentSessionResolver` 现注释的冲突已裁定):** 令牌存在、仅仅是过期 → `TOKEN_EXPIRED`;其余全部(没带头 / 格式不对 / 查不到 / **已吊销**)→ `UNAUTHORIZED`。
>
> 🔴 **「已吊销」不单独成档** —— 那才是真的送信息:它等于告诉持有者「这个令牌曾经是真的」,而 `POST /tokens/revoke-all` 存在的意义正是让被吊销的一方什么都不知道。「过期」不同:持有它的人本来就知道它曾经有效,分出来不泄露任何东西。
>
> 需要的签名:`TokenService.check(String) → TokenCheck`(密封,`Valid`/`Expired`/`Invalid`),`verify` 保留。
...
| `B0-4` 鉴权默认值 | §五:过滤器 + 五行白名单,`401` 拆两档,默认拒绝的判据 | ✅ |
...
| KUBI-93 | `M4` 出口与令牌 | `B0-4` 只读令牌四道锁 + `B0-6` 幂等(`POST /ai/ask` 是三处之一) |
...
| # | 事 | 谁 | 什么时候 |
|---|---|---|---|
| 1 | **`web/` 有没有在读 `total` / `hasMore`** —— 读了的话删字段会打断前端,而那同时是 `U7.6` 的另一处违反 | `M1`(KUBI-90)开工第一件事跑 `grep -rn 'hasMore\|\.total' web/src`;读了就报回本组,**不自己改前端** | stage 2 内 |
| 2 | **`/api/**` → `/api/v1/**` 什么时候迁** | 🔴 **项目经理排,不是技术侧定。** `接口契约` §1.1 已登记这处落差并写明「迁移时点由项目经理排」 | `B0-4` 动手前 |

🔴 **第 2 条有一个必须点破的连带后果:本文的白名单常量按 `/api/v1` 写,而代码里十个 controller 全部挂在 `/api/**` 下。** 迁移前那五行匹配不到任何东西 —— **这是本文唯一一处「写了但今天不生效」的东西**。`B0-4` 落地时二选一:要么同时迁前缀,要么白名单临时写 `/api`。**不许两个前缀都写一遍** —— 那会让「匿名入口的全集在这一处数得清」当场失效,而那正是白名单存在的全部理由。

**为什么第 2 条不由技术侧自己定:** 迁前缀会同时打断 `web/`、`shell/` 的 `/api` 代理、以及任何已经在跑的端。它是一次一次性的、跨四个端的改动 —— `接口契约` §1.1 已经把它归给项目经理,本文不越过去。
