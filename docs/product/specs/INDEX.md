...
> 🔴 **下表有四行的码名与服务端实际返回的对不上**，照本表实现会让这四种处境一律落到「未知错误」行：
> `SMS_CODE_WRONG` → **`CODE_WRONG`**、`SMS_CODE_EXPIRED` → **`CODE_EXPIRED`**、
> `SMS_LOCKED` → **`PHONE_LOCKED`**、`PHONE_TAKEN` → **`BIND_REFUSED`**。
> **按惯例此处只加注，下表原文不改** —— 改哪一侧、什么时候改，由 §十 那一侧统一定。
