...
| 8 | GET | `/api/v1/tokens` | `full` | — | 🔴 **不分页,返回全集** | `auth` + `app` |
| 9 | DELETE | `/api/v1/tokens/{tokenId}` | `full` | 🔴 **端点自身幂等** | — | `auth` + `app` |
...
### 2.2 幂等:M4 占五处中的两处,而 `B0` 那张表只写了三处
...
🔴 **裁定:以端点级契约(§6.3 / §6.6)为准,`B0` §7.3 那句「三处」更正为五处。** 理由不是谁大谁小,是**那两行签名里逐字写着这个头**,而横切归纳是后写的。更正原文见 §13.1。
...
| 档 | HTTP | `code` | 备注 |
|---|---|---|---|
| 未登录 / 登录过期 | `401` | `UNAUTHORIZED` / `TOKEN_EXPIRED` | 两档由 `B0` §5.3 的 `TokenCheck` 给出,M4 只消费 |
...
### 9.6 `GET /tokens` 与 `DELETE /tokens/{tokenId}`
...
| `TOKEN_EXPIRED` | 401 | 全部十行 | 登录过期(`B0` §5.3 拆的两档) |
...
见 §2.2。**裁定:以端点级契约(§6.3 / §6.6)为准,更正为五处。** 更正原文见 §13.1。
...
### 增量 1 · §三 鉴权总表那一列「幂等」,取值补到五行

> **`B0` §十五 增量 2 说「今天要标 `Idempotency-Key` 的只有三行」,实读本文 §6.3 与 §6.6 得到五行。** 补上的两行是:
>
> | Method | Path | 幂等 | 为什么 |
> |---|---|---|---|
...
>
> **三种键的保留期由各模块定,M4 这三处是:`/export/jobs` 30 分钟(与作业同寿)、`/ai/ask` 24 小时、`/tokens/revoke-all` 24 小时。**
...
> ⚠️ **2026-09-03(`M4`):本节「实际要用 `Idempotency-Key` 的三处」实读为五处。** 漏掉的两处是 `POST /export/jobs`(§6.3 签名里逐字写着这个头)与 `POST /tokens/revoke-all`(§6.6 同上,并命中本节自己那条「不可逆改变账号状态」)。
