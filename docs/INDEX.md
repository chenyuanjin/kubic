...
**不进 [`文档规范`](文档规范与目录.md) §2.1 台账、不领简称、不进任何阅读路线、不进份数与行数口径**
（计数命令一直带着 `-not -path '*/research/*'`）。要看端与栈的结论读 [`多端选型`](technical/多端选型与端矩阵.md)，不要读这里。

| 原件 | as-of | 它是谁的输入 |
|---|---|---|
| [九端跨平台选型手册](research/2026-08-30-九端跨平台选型手册.md) | 2026-08-30 | `多端选型` |
| [小程序与H5多端框架现状](research/2026-08-30-小程序与H5多端框架现状.md) | 2026-08-30 | `多端选型` · `壳技术方案` |
...
**没有一条路线要求读完全部 30 份。** 全读一遍是 11,796 行（`find docs -name '*.md' -not -path '*/research/*' | xargs wc -l` 减去作废的 `产品路线图`），等于什么都没读。
**这里数的是活跃文档**；磁盘上是 31 个文件 / 12,015 行（另有 `research/` 两份原件 1,047 行，不进这个口径），多出来的那一份是作废但按 `文档规范` §4.3 不删的 `产品路线图`。份数与行数的唯一真源仍在 [`文档规范` §2.5](文档规范与目录.md)。
...
其他目录：`misc/`（杂项，两道闸见 `文档规范` §3.2）· `research/`（外部调研原件，非本项目文档，不进阅读路线；规则与两份原件的链接见 §零）。
