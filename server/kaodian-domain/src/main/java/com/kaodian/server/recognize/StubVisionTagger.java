 ...
 * docs/data/识别链路选型.md 坑一:「OCR 出错是漏字,LLM 出错是编造一个不存在的考点。而错标会让覆盖度失真,
 ...
 *
 * <p>换成真实实现(多模态一步到位,docs/data/识别链路选型.md §一)时,只需要多一个 {@link VisionTagger}
 * 实现类:内部把 {@code image} 转 base64 <b>内联</b>发送(🔴 禁用厂商 Files API,docs/data/识别链路选型.md 坑二),
