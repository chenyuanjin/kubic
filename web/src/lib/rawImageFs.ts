 ...
 *
 * `docs/21 §1.1` 冻结的那句:**壳带来的是 `RawImageBackend` 的第二个实现,不是第二套逻辑。**
 ...
 * 的 Rust 测试守着</b>,两半在线协议上碰头,见 `docs/21 §9.2`。
 */
...
/* ========================================================================== */
/* 线协议 —— 逐字对着 docs/21 §9.1 那张表,壳侧 raw_image_store.rs 是同一张表     */
/* ========================================================================== */
 ...
 * <p>双下划线开头是给读代码的人看的:<b>它不是产品 API</b>,不进 `docs/10` 的接口表。
 */
...
export const fsRawImageBackend: RawImageBackend = {
   ...
   * 再原子重写索引;中间崩溃留下的孤儿字节文件由启动清理删掉,见 `docs/21 §2.4`)。
  ...
  async read(id: string): Promise<StoredRawImage | null> {
    ...
    //    而 docs/10 §8.1 禁令 4 是「不做任何形式的图片分享/外链」。
