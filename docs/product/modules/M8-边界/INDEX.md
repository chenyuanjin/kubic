...
| 单元 | 管什么 | 关键决策 |
|---|---|---|
| [`U8.1` 「这个产品不做什么」这一页](U8.1-这个产品不做什么这一页.md) | 边界给用户看的那一份 | 五句是产品里**唯一一份**边界全文，三处消费者逐字同源，`scripts/boundary-copy-check.py` 守着；🔴 **首启展不展示，两份规格结论相反，登记待裁**（§七 `M8-G1`） |
...
| # | 缺口 | 缺的是哪一半 | 该谁定 |
|---|---|---|---|
| `M8-G1` | **边界全文那一页在首启展不展示，两份规格结论相反**：`设置与原图` §3.3 判「不展示」且 `A-27` 仍在生效，`首次使用与权限申请` §1.3.3 定成不可跳过的第一屏 | 缺的是一次裁定，不是缺信息 —— 两边理由都完整，只是结论相反 | 🔴 **人裁定。** 原文与行号见 `U8.1` §五 |
