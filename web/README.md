...
| 考点自行命名 | 考点管理里<b>没有</b>「从机构导入考点体系」这个入口;`api/types.ts` 里连对应的请求类型都不存在(R-07 / docs/04 §1.2) |
| 只有三层 | 新增只有「新增题型」和「在题型下新增考点」两种。`CreateNodeRequest` 里只有 `groupCode`,没有 `parentNodeCode` —— <b>结构上长不出第四层</b>(01 §2.5) |
