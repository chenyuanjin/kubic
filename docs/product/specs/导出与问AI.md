...
| 元素 | 取值来源 | 为空时 | 可点吗 | 禁用条件 | 无障碍朗读文本 |
|---|---|---|---|---|---|
| 区标题「只读令牌」 | 固定 | 阶段 2 之前**整区隐藏** | ❌ | —— | 「只读令牌,区域标题」 |
...
| `X-64` | ~~未登录 / 本机模式进 `S-EXP`~~ | **已裁定 · 作废**(2026-09-02:没有本地模式,进得到 `S-EXP` 的一定已登录;阶段 2 之前区四整区隐藏见 §3.5) | —— | —— | —— | —— |
...
| `A-54` | 未登录(断言按新形态反过来写,2026-09-02 用户裁定) | 用深链或外部唤起直接打开 `S-EXP` | 🔴 落到登录门,**进不到 `S-EXP`**;四个区一个都不渲染。阶段 2 之前区四整区不渲染(不是灰按钮、焦点顺序里没有它)由 §3.5 保证 |
