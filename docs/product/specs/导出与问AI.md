...
| 通道 | 是什么 | 要账号 | 要额度 | 阶段 | 性质 |
|---|---|---|---|---|---|
| **只读令牌(MCP)** | 签发一串只能读的凭证,让用户自己的 AI 客户端直接读他的盲区 | ✅ | ❌ | 阶段 2 后 | 营销资产,不是收入线(`模块 E · 出口层` §2.5) |
...
| 出口 | 未登录·本机 | 已登录·免费 | 已登录·付费 | 无骨架 | 完全离线 |
|---|---|---|---|---|---|
| 导出文件(本机数据) | ✅ | ✅ | ✅ | ⚠️ 只能导记录,导不出盲区 | ✅ **必须可用** |
| 导出文件(云端数据) | — | ✅ | ✅ | 同上 | ❌ 见 §十四 |
| 复制给 AI | ✅ | ✅ | ✅ | ❌ 整区隐藏 | ✅ 本机数据可用 |
| AI 代发 | ❌ 点击进登录 | ✅ 额度内 | ✅ 额度内 | ❌ | ❌ |
| 只读令牌 | ❌ 整区隐藏 | ✅ 阶段 2 后 | ✅ | 不相关 | ❌ 签发/吊销都要网 |
...
flowchart TD
    ...
    subgraph SCROLL["滚动区(自上而下四个区,顺序不可调)"]
        ...
        Z3["区三 · 让 AI 直接回答(阶段 2 后)<br/>剩余次数 + 发送按钮 + 回答区"]
        Z4["区四 · 只读令牌(阶段 2 后)<br/>令牌列表 + 签发按钮"]
    end
...
| 「发送」按钮 | —— | —— | ✅ | ①未登录时**不禁用**,点击进登录 ②生成中 ③预览框为空 ④离线 | 「把上面那段文字发给 AI,按钮{,不可用:{原因}}」 |
...
| 元素 | 取值来源 | 为空时 | 可点吗 | 禁用条件 | 无障碍朗读文本 |
|---|---|---|---|---|---|
| 区标题「只读令牌」 | 固定 | 未登录时**整区隐藏** | ❌ | —— | 「只读令牌,区域标题」 |
...
| 受限 | 未登录 / 额度为 0 | 不受影响 | 不受影响 | 见 §九 | 未登录整区隐藏 |
...
🔴 **这四步里没有任何一步能被付费状态改变。** 未登录的本机用户走的是同样的四步,拿到的是同样格式、同样字段、同样没有水印的文件。
...
| `account_mode` | `local`(本机)/ `cloud`(已登录) | 端状态 | 否 | 二选一枚举 |
...
| 用户想看自己记过什么怎么办 | 在 `S-TL` 时间线里看,那是本机/本账号内的浏览,不产生一份可传播的文件 |
...
🔴 **这条路径全程零网络请求、零额度、零登录。** 文本在端上用本机已有的数据拼出来 —— 这就是「永不移除」在实现上的形态:
...
| `X-34` | 未登录导出后又登录,数据范围变了 | 页内提示 | 「登录之后能导的东西变多了,要不要再导一份」 | 再导 / 忽略 | —— | ✅ |
...
| # | 触发条件 | 界面表现 | 上屏原文文案 | 用户下一步 | 后端行为 | 数据留住了吗 |
|---|---|---|---|---|---|---|
| `X-06` | 未登录点代发 | 受限 | 「这一步需要先登录」 | 登录后自动重试 | 无请求 | ✅ |
...
| `X-53` | 代发进行中令牌过期 | 静默续期;续期失败按未登录 | 「这一步需要先登录」 | 登录后重试 | `POST /auth/refresh` | ✅ |
...
| `X-64` | 未登录 / 本机模式进 `S-EXP` | 区四**整区隐藏**,不显示灰按钮 | —— | —— | —— | ✅ |
...
| # | Given | When | Then |
|---|---|---|---|
| `A-01` | 未登录的本机用户,本机有 96 条记录 | 连续点「导出」20 次 | 20 次全部成功;屏幕上从未出现次数、剩余、限制、升级相关的任何字样;20 个文件字节级别一致(除文件名里的时间) |
...
| # | Given | When | Then |
|---|---|---|---|
| `A-40` | 未登录用户 | 点「发送」 | 进 `S-LOGIN`;登录成功后**回到 `S-EXP`,范围与文本原样保留,并自动重试一次** |
...
| `A-54` | 未登录用户进 `S-EXP` | 观察区四 | 🔴 **整区不渲染**,不是灰按钮;焦点顺序里没有它 |
