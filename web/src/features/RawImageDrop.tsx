...
/**
 * 「拖张图进来」—— docs/08 `1.1.3` 在界面上的三处表达。
 *
 * <h2>docs/08 的 UI 审核项逐条落在哪</h2>
 *
 * <table border="1">
 *   <tr><th>审核项</th><th>在这一屏的哪里</th></tr>
 *   <tr><td>① 同意点</td><td>{@link ConsentGate} —— <b>第一次</b>导图时挡在前面,
 *       明写「存在这台设备上、N 小时后自动删除」。不点「知道了」就一张图都不收</td></tr>
 ...
 *       但 docs/10 §8.1 禁令 4 是「不做<b>任何形式</b>的图片分享/外链」,
...
export function RawImageDrop({
  ...
  return (
    <>
      <GroupHeader
        title="拖张图进来"
        right={cached.length > 0 ? `本机 ${cached.length} 张 · ${hours} 小时后自动删` : '你自己截的图'}
      />
        ...
        <div className="mt-2">
          {problem !== null ? (
            ...
            <Note warn>
              ...
              <b className="font-normal text-t2">自动删除</b>;存进去那一刻就写好了到期时刻,不是删的时候才算。
              <br />
 ...
 *
 * docs/08 的审核项写的是<b>同意点</b>。一条常驻说明只保证「写了」,
 * 而这条红线的性质是 docs/01 §2.3 那句「第一天不定,后面改不回来」——
    ...
    <div className="rounded-sm border border-acid/40 bg-bg3 px-3 py-2.5">
      ...
      <ul className="mt-2 space-y-1 text-[12px] leading-[1.75] text-t2">
        <li>
          ...
          <b className="font-medium text-tx">{hours} 小时后自动删除</b>。存进去那一刻就写好了到期时刻。
        </li>
