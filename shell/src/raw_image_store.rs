...
//! ```
//!
//! 🔴 **目录本身就是红线的物理落点**(`docs/21 §3.4`):它落在应用数据目录,
...
impl StoreError {
    fn io(e: std::io::Error) -> Self {
        ...
        // docs/10 §8.2 明说路径也是设备信息,而这个字符串会进错误体。
...
impl RawImageStore {
    ...
    /// 而 `docs/21 §3.2` 把允许存在的差异穷举成了两处。两个实现行为一致比这一侧更聪明重要。
    ...
    /// 所以选**字节先走**的那一边;留下的元信息残行是 `docs/21 §2.4` 已经判过不危险的那一种。
    ///
    /// 删不存在的 id 不算错(幂等)。
...
mod tests {
    ...
    fn delete_removes_bytes_before_the_index_so_a_crash_never_leaves_the_bytes_behind() {
        ...
        // 索引里还留着那一行,而它 read 不出来 —— docs/21 §2.4 判过的那种「不危险的残留」。
        fs::remove_dir_all(dir.join("index.tmp")).unwrap();
        assert_eq!(s.list().unwrap().len(), 1);
    ...
    fn put_overwrites_the_same_id_instead_of_duplicating_it() {
        // 与 IndexedDB 实现行为一致 —— 差异要可穷举,而 docs/21 §3.2 只列了两处。
