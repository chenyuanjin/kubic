...
| 标识 `int64`,JSON 里字符串 | §1.1 / `B0-2` | `jobId` / `nodeId` 是 `long`,响应里是 `"9021"` 不是 `9021`。⚠️ **2026-09-04 更正(codex 审核轮):`tokenId` 不在这一行里** —— `接口契约` §1.1 第三行早已把**令牌标识**定为**不透明字符串**,是 int64 规则的唯一例外(`M5` §十三 增量 4)。本文上一版把它写成 `long`,与契约和 `M5` §9.7 同时冲突 |
| 空值:没有就不出现这个 key | §1.1 | `downloadUrl` 只在 `state=SUCCEEDED` 时出现;`failure` 只在 `state=FAILED` 时出现;`lastUsedAt` 从没用过时**整个 key 不出现**,不是 `null` |

### 2.2 幂等:M4 认领两处,而 `B0` 那张表当时只写了三处
...
四份模块设计各数出一个数(`B0` 三行 / `M2` 四行 / `M4` 五行 / `M7` 五行),KUBI-96 stage 3 已按同一条判据取**并集**并落进 `接口契约` §3.3(⚠️ 那次并集后来又发现漏了 `M5` 一行)。
...
### 9.4 🔴 令牌 id 是**不透明字符串** `tokenId`,不是 `tokenHash`,也不是 `long`
...
| ✅ | 独立的**不透明字符串** `tokenId`,与 `tokenHash` 一一对应但互不可推 | 端拿到的东西**推不回 `tokenHash`**,`U4.3` §2.1「不导出令牌指纹」保住 |

⚠️ **2026-09-04 更正(codex 审核轮):本行上一版写的是「`long tokenId`」,理由是「`B0-2` 定的 id 一律 `long`,顺带满足」。**
🔴 **那个「顺带满足」正好把这条端的安全属性做反了。** `接口契约` §1.1 与 `M5` §十三 增量 4 的理由逐字是:
「改成 `int64` 等于给令牌加一个**可枚举的序号**,而可枚举意味着『这个账号有几条令牌』变成一个能被数出来的事实」。
本文原来的论证只盯着「不许把 `tokenHash` 发出去」,**不透明串同样满足那一条,而且不额外送出条数** —— 两个目标不冲突,`long` 是三个选项里唯一同时满足前者、破坏后者的那个。
**两个令牌面(`GET /tokens` 登录设备 · `GET /tokens/readonly` 只读令牌)读同一张 `AccessToken` 表,类型必须同一个。**
...
⚠️ **2026-09-03 更正(KUBI-89 审核轮):不是「更正为五处」,是取并集** —— **行数只在 `接口契约` §3.3 那张表上数**(它此后又从八行更正为九行,正好说明本文不该记这个数);另见 `INDEX.md` §四 冲突 1。更正原文见 §13.1。
...
🔴 **本段不再复述行数** —— 照上一版合并会把 §3.3 那张表改回五行。下面两行是 **M4 认领的那两行**(即 §3.3 的第 6、7 行),不是全集。
...
>
> ⚠️ **2026-09-03 二次更正(KUBI-89 审核轮):不是五处,是取并集,而行数只在 `接口契约` §3.3 那张表上数(它此后又从八行更正为九行)。** `M2` 另有 `POST /records/{id}/tags/suggest`,`M7` 另有 `close` 与 `receipt/verify`。
...
>
> 🔴 **令牌对外的标识是不透明字符串 `tokenId`,不是 `tokenHash`,也不是 `long`(`接口契约` §1.1 第三行)。** `tokenHash` 是账号内部标识,而 `U4.3` §2.1 明写「不导出令牌指纹」;把它发给端还会让后续每一个「按 hash 查/改」的端点都收一个来自客户端的哈希。⚠️ 现状 `SessionDto.java:31` 与 `RevokeSessionRequest` 是反例,归 `M5`。
