...
/**
 * 「我已掌握」—— docs/10 §5.2 的 {@code user_assertion} 表。
 *
 * <h2>🔴 它<b>不</b>进覆盖度的分子。这是这个 record 存在的全部理由</h2>
 *
 * 01 §5.2 的原话是:「录入完整度决定诊断质量……<b>『我已掌握』按钮是补丁不是解法</b>。」
 ...
 * 而这个产品唯一的那个数字必须只由「碰过」推出来(01 §2.2 能力边界:有没有、几次、多久前)。
 * 一个能靠自我声明刷高的覆盖率,和一个没有覆盖率的产品,价值是一样的。
 * <p>
 * 落到代码上就是三件事,写在 {@code CoverageService} 里:
 * <ol>
 *   <li>覆盖率的<b>分子不变</b> —— 断言不让任何考点变成「碰过」</li>
 *   <li>盲区榜<b>排除</b>它 —— 用户不想再被提醒,这是他按下按钮时要的东西(docs/10 §6.4)</li>
 *   <li>概览里<b>单列一格</b> —— 「你声明掌握了 N 个」,与覆盖率并排但不相加(docs/10 §6.4)</li>
 * </ol>
 *
 ...
 * </table>
 * 归档那一侧的风险有单独一条记录({@code docs/08} §四 {@code R-49}:「归档可无声刷高覆盖率」,
