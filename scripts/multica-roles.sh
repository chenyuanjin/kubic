...
#
#   现有 8 个 agent 是【轨道导向】(docs/总路线图 三条线),
...
#   不做的事:不建 autopilot(docs/交付工作流 §八:webhook 在自托管上未验证前,一切走 CLI 回写)
# ─────────────────────────────────────────────────────────────
set -uo pipefail
...
GUARD='
━━━ 硬约束(违反即停,出处 docs/决策记录 与 docs/总路线图 §四)━━━
...
📌 你的结论是【输入】不是【判决】:议题最多推进到 in_review,永远不能置为 done(docs/交付工作流 §9.1)。
📌 判决写 metadata 五个键(verdict / blocking_count / repro_cmd / redline_hit / escalate),
   散文写 comment。没有可复现 repro_cmd 的判决无效(docs/交付工作流 §9.2)。
📌 分支名必须是 KUBI-<n>-<slug> —— PR↔议题关联就靠这个字符串(docs/交付工作流 §9.3)。'
...
职责:
- 盯 docs/总路线图 三条线的父子议题状态,识别阻塞与依赖倒挂
- 维护 docs/详细排期 的逐周排期;排期纪律:周六=写代码(连续注意力),工作日晚上=打标(碎片),周日=复盘
...
- 维护 docs/模块地图 的模块地图与 docs/产品脑图 的脑图;新功能先写清楚它服务于哪个关卡
  (原写的是 docs/产品路线图 产品路线图 —— 该文档 2026-08-31 已作废,见 docs/文档规范 §2.4-④)
...
职责:
- 维护 docs/技术架构(技术架构与接口契约)与 docs/后端详设(后端系统设计与组件接入)
...
- 唯一允许的构建入口是 ./server/build.sh,直接 ./mvnw 会走公司私服(docs/技术架构 §1.3)

交接:你从产品经理接流程;把契约交给后端与跨端前端;契约变更必须同步 docs/技术架构。'
...
mk "后端与AI打标" "$RT_CLAUDE" claude-opus-5 '你负责 server/ 后端实现与【自动打标能力】。docs/总路线图 标着「关卡 1 的成败在这里」。
...
- docs/决策记录 §2.4:响应式 Web 为主;小程序曾被推翻,若回归只做采集入口不做分析
