...
| `R-04` 原图不上云(**含所有厂商暂存**,`R-52`) | ① 出站 HTTP host 白名单测试,厂商图片暂存端点必须不在其中;② 原图 TTL 到期**转归档**的注入时钟测试(**2026-08-29 改判**:到期不再删除,见 `16`;按旧文写成「断言字节已消失」会把改判判成违规);③ 断言代码中不出现 `X-DashScope-OssResourceResolve` | server 测试 | ✅ 建议做 |
