...
>
> **屏幕级规格在 [`记一次学习`](../specs/记一次学习.md)** —— 那一份写到可以直接做 UI 与前后端开发,本文只写产品判断。
>
> ⚠️ 阶段 0 的判据就是自己用两周、每天记两个数
...
`总路线图 · 脑图与父子待办` 编号 `1.1.1` `1.1.2`；离线队列 `1.1.2b`；原图 `1.1.3` 🔴`R-04`；「记录动作永不失败」`1.3.7.1`（全程）。
...
| B-2 | **删记录的撤销形态未定** | 前端有动作、后端有契约，**中间那一格（能不能撤销、撤销窗口多长）没人定过** | 产品；现在可以先不定 —— 自用期间误删可以直接改库 |
