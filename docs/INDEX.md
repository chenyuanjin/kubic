...
│   ├── 商业化与额度设计.md
│   ├── 用户价值与主流程.md                 L1 · 旅程 J0–J6，每个模块必须填得出节点号
│   ├── modules/                        L2 · 九个域 51 个能力单元 —— 「为什么这么设计 + 长什么样」
│   │   ├── INDEX.md                      模块地图：51 单元清单 + 状态机总表 + 跨模块不变量 I-1–I-6
│   │   ├── 线框与交互规格约定.md            §六/§七 两节的共享写法约定（记号表 · 必答五项 · 红线自查）
│   │   └── M0-…/ … M8-…/                 九个域，每域一份 INDEX.md + 若干 U?.?.md（合计 51 份）
│   └── specs/                          L3 · 用户侧功能规格 —— 「屏幕长什么样、点下去发生什么」
│       ├── INDEX.md                      总纲：屏幕地图 + 全局交互约定 + 错误码映射
│       ├── 登录与账号.md · 记一次学习.md · 看盲区.md
│       ├── 打标与未分类.md · 导出与问AI.md · 首次使用与权限申请.md
│       ├── 全局组件与交互模式库.md · 端到端验收用例集.md · 无障碍与多端适配.md
│       └── 额度与购买.md · 设置与原图.md　　（合计 12 份）
├── technical/                        技术契约与详设
│   ├── INDEX.md                        技术架构与接口契约
│   ├── 接口契约-签名与错误码全集.md      签名 / 字段类型 / 错误码全集 / 分页与幂等
│   ├── 后端系统设计与组件接入.md
│   ├── 壳技术方案-Tauri2包现有Web工程.md
│   ├── 多端选型与端矩阵.md
│   ├── 原图存储-判据层与存储层.md
│   └── backend/                       后端模块实现设计（2026-09-03 新增）
│       ├── INDEX.md                     唯一真源位置 + 目标态 ER 图 + 模块依赖图
│       ├── B0-平台底座与横切契约.md         ← 先读这一份，六份模块设计的公共前提
│       ├── M1-记录采集与离线补传.md · M2-打标管线与模型接入.md
│       ├── M3-骨架与覆盖度差集.md · M4-出口与只读令牌.md
│       └── M5-账号与登录通道.md · M7-额度与订单.md　　（合计 8 份）
...
**没有一条路线要求读完全部 103 份。** 全读一遍是 44,813 行（`find docs -name '*.md' -not -path '*/research/*' | xargs wc -l`），等于什么都没读。

⚠️ **2026-09-03 重算：原文写的是「30 份 / 11,796 行」，实际是 103 份 / 44,813 行 —— 差了三倍多。**
涨的几乎全在 `product/`（78 份 / 33,143 行，其中 51 份是能力单元）。
**这个数字在增删文档时必须同一次改动内重算** —— 上面那条命令就是判据，它一直在，只是没人跑。
...
| **写后端代码** | `CLAUDE.md` 红线表 → [`技术架构与接口契约`](technical/INDEX.md) → [`接口契约：签名与错误码全集`](technical/接口契约-签名与错误码全集.md) → [`后端系统设计与组件接入`](technical/后端系统设计与组件接入.md) | ~4014 |
| **落一个后端模块** | [`后端模块设计索引`](technical/backend/INDEX.md) → [`B0 平台底座与横切契约`](technical/backend/B0-平台底座与横切契约.md) → 该模块那一份（`M1`…`M7`）。🔴 **`B0` 不是可选前置** —— 六份模块设计的存储形态、ID 类型、租户列、鉴权默认值全在它那里 | ~7,268 |
...
### L2 · 详设与查阅（8 份，~5325 行；只在真要动那一块时打开）

`商业化与额度设计` `接口契约：签名与错误码全集` `后端系统设计与组件接入` `自动化交付工作流 · 基于 Multica` `Agent 框架与能力边界` `Multica 操作备忘` `壳技术方案：Tauri 2 包现有 Web 工程` `原图存储：判据层与存储层` —— 其中 [`后端系统设计与组件接入`](technical/后端系统设计与组件接入.md) 单份 1,738 行，是全仓最大的一份，
**它是 `技术架构与接口契约` 的下一层**，不要当入门读物。

**反面材料**（`已证伪方向与调研结论` `思考模式与选择框架`，~350 行）不属于任何一档，它们在你**要提新方向或做判断时**读，平时不读。

> 三档 + 反面材料 + 本文 = **103 份 / 44,813 行**（`decisions` 4/806 · `execution` 3/1,378 · `product` 78/33,143 · `technical` 6/5,953 · `data` 5/1,526 · `ops` 2/721 · `ops-console` 1/63 · `misc` 2/336 · 根 2/887），与 §一 的总数对得上（2026-09-03 重算）。**这些行数在增删文档时同一次改动内重算**
...
| **展开** | [`用户侧功能规格`](product/specs/INDEX.md) **十三份**逐屏规格 · [`模块地图`](product/modules/INDEX.md) 的**九个域 53 个能力单元**（`M0`–`M8`；旧的「模块 A–G」七域口径 2026-09-02 已废） · [`商业化与额度设计`](product/商业化与额度设计.md) 商业化与额度 · ⛔ **H 组无展开文档**（原 [`产品路线图与用户共创`](misc/产品路线图与用户共创-已作废.md) 已作废） | [`接口契约：签名与错误码全集`](technical/接口契约-签名与错误码全集.md) 签名与错误码 · [`后端系统设计与组件接入`](technical/后端系统设计与组件接入.md) 后端详设 · [`多端选型与端矩阵`](technical/多端选型与端矩阵.md) 端矩阵 · [`Agent 框架与能力边界`](misc/Agent框架与能力边界.md) Agent 框架（收纳在 `misc/`） |
