...
**看到差异 ≠ 了解现状。** 这条单独列,因为它让一个 agent 白干了一趟。

2026-08-30 我用 `git diff origin/v1...origin/KUBI-65` 判断「三份 crate 并存」,
据此建了一个合并议题派给开发组。实际上合并早已完成 —— `00925ec` 当时就在 `origin/v1` 上,
而我量的是**分支之间**的差异,不是**主干上有没有**。

派活之前的固定动作:

```bash
git fetch origin
git log --oneline -3 -- <相关路径>     # 主干上这块最近发生过什么
git ls-tree -r --name-only origin/v1 -- <路径> | wc -l
# 要判断某分支是否已并入,量的是「v1 与它的差异」,不是「它相对某个基点的差异」:
git diff --name-only v1 origin/<branch> -- <路径>   # 0 = 已并入
```
...
**下达指令 ≠ 事情发生;看到差异 ≠ 了解现状。动手前查现状,改完查结果。**
