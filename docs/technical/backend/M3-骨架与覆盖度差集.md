...
| 项 | 契约 |
|---|---|
| 鉴权 | `full`。🔴 **不进 §三 那张七行匿名白名单** —— 一个匿名可读的口径端点会立刻被端当成「登录前就能拿到默认值」，而那正好是把默认值搬回端上的路 |
...
| 鉴权 | `full`。🔴 **不在 §三 那张七行匿名白名单里**。只读令牌 `POST` → `403 READONLY_TOKEN` |
...
| `ACCOUNT_DEACTIVATED` | 401 | 本域全部 | 账号已注销，旧令牌仍在被使用 | 「已注销」（`U5.5`）。🔴 **归 `B0-4` / `M5` 的鉴权过滤器抛，M3 域内端点不主动抛这一档** —— 列在这里是因为端要认它，不是因为它归 M3。`接口契约` §1.2 的 `401` 是**三档**不是两档 |
...
## 十四之二、实现回写（`KUBI-103` 落码时发现，2026-09-04）

> **本节是实现过程中发现的契约错漏，按共同约束第 6 条回写。** 与 §十四 一样，要合进上游那份契约的原文归 stage 3；标 ⚪ 的仍然不裁定。

### 回写 1 · 判据 18 的 `grep` 会被 `ErrorCode.IN_PROGRESS` 命中（假阳性）

§7.1 那条黑名单含 `progress`，而 `B0` 落的 `ErrorCode` 枚举里有一个 `IN_PROGRESS(409)`（同一幂等键的上一次仍在进行，`接口契约` §十 有这一行），它住在 `api/dto/common/` —— 正好是判据 18 的扫描根之一。

**这是一次真实的假阳性**：`IN_PROGRESS` 不是掌握度指标，它是一个 HTTP 409 的错误码；而它由 `B0`（`7abdfa1`）引入，不是 `M3` 加的。

**判据改为**（其余不动）：末尾的过滤链再加一段，排除这一个已知标识符 —— 与那条 `grep -vE` 去注释行是同一类修正，都是「让判据只对准它要守的东西」：

```bash
  | grep -vE '^[^:]+:[0-9]+: *(\*|//|/\*)' | grep -v 'IN_PROGRESS' ; echo "expect: 0 hits"
```

⚠️ **不要反过来去改 `ErrorCode`**：那个码是 `接口契约` §十 的一行，改它是改契约；而这里要修的只是一条扫描规则。

### 回写 2 · 判据 13 与判据 20 的 `grep` 缺 `src/main` 收窄与去注释行

两条判据今天写的是 `grep ... server/`，于是它们同时扫到 **测试代码** 与 **本仓自己的合规注释**：

| 判据 | 命中什么 | 是不是真的 |
|---|---|---|
| 13（没有服务端「已跳过」标记） | `ExamProfileController` / `ExamProfileDto` 里写着「服务端因此没有 `profileSkipped`」的注释；`ExamProfileApiTest` 里那条**断言这些字段不存在**的用例 | ❌ 全是假阳性 |
| 20（归档不是一个开关） | `CaptureServiceTest` / `CandidateRecallTest` 里的**测试夹具构造器** `withArchived(Syllabus, String)` | ❌ 假阳性，它不是查询参数 |

`CLAUDE.md` §Delivery 已经写死这一条：「黑名单不得命中本仓自己的合规注释，那些注释用否定句写成，句子里必然含着被禁的词」。**判据 13 / 20 改为**：

```bash
grep -rniE 'profileSkipped|hasSeenProfile|promptShown|skipFlag' server/*/src/main/java/ \
  | grep -vE '^[^:]+:[0-9]+: *(\*|//|/\*)' ; echo "expect: 0 hits"
grep -rniE 'includeArchived|showArchived|withArchived|hideArchived' server/*/src/main/java/ ; echo "expect: 0 hits"
```

收窄到 `src/main` 不是放水：这两条守的都是**生效代码里有没有这个东西**，而一条断言它不存在的测试<u>必须</u>写得出那个名字。

### 回写 3 · 🔴 `X-Config-Fallback` 必须进 CORS 的 `allowedHeaders`，否则它在跨域这条路上永远发不出去

§3.2 定义了这个诊断头，但没说它要过 CORS。`ApiCorsConfig` 的 `/api/v1/**` 那条 mapping 只放行 `Content-Type` 与 `Authorization` —— 浏览器**在预检就拒了**，端拿不到任何可以上报的信号。

**后果比「少一个头」严重**：这个头本身就是「口径拿不到」的上报通道，它自己发不出去就等于**偏离登记这条链路无声地不存在**，而没有任何东西会报错。壳（Tauri，loopback 同源代理）不受影响，`web/` 直连的那条路受影响 —— 也就是说这个缺口**只在一种端上出现**，最难被发现。

**§5.4 追加一行**：

| 项 | 约束 |
|---|---|
| 跨域 | 🔴 头名必须出现在 `/api/v1/**` 那条 CORS mapping 的 `allowedHeaders` 里。少这一行，这个头在浏览器直连时**永远发不出去，而且端侧静默失败** |

### 回写 4 · `statsAsOfYear` / `lastSeenYear` / `avgPerPaper` 今天在骨架层没有来源

§9.1 / §9.4 列了这三个字段，而 `Syllabus.Node` 今天只有 `(code, name, recent5yCount, archived)` —— 三个字段**一个都没有存放的地方**。

**这不是一个落差，是「没数过」那一档本身**（§二）：没有来源 → key 不出现 → 界面写缺失文案。契约不需要改，**但要在 §9.1 / §9.4 就地加一句注记**，免得下一个人把它读成「后端漏实现了」：

> ⚠️ 这三个字段的来源在**骨架数据线**（`数据线 · 骨架原料的获取与隔离`），不在 `M3`。骨架长出它们之前，这三个 key 恒不出现 —— 那正是契约要的形态，不是待补的 0。

### 回写 4b · ⚠️ `recent5yCount` 的「没数过」那一档今天到不了，`hasStatsOnly` 因此是个空转参数

与回写 4 同源但更要紧一格：`Syllabus.Node.recent5yCount` 是一个**原始 `int`**，所以从 `compute` 出来的每个 `NodeCoverage` 的 `recent5yCount` **恒非 `null`**。后果两条：

1. §二 那张表里 `recent5yCount` 的「key 不出现 →『这个考点没有出现次数记录』」这一档**今天不可能出现**——`0` 与「没统计」在库里就是同一个值。
2. `GET /coverage/blindspots?hasStatsOnly=true` 因此**永远筛不掉任何一行**。

**代码没写错**：`CoverageService.blindSpots` 的过滤条件就是 `recent5yCount() == null`，骨架能表达「没统计」的那天它自动开始生效。**错的是骨架层存不下这一档**。

⚪ **不在本议题裁定**：把 `Syllabus.Node.recent5yCount` 改成 `Integer` 会连带 `FileSyllabusStore`（13 处）、`SyllabusLoader`、`CandidateRecall`、`SetFrequencyRequest` / `CreateNodeRequest` 与骨架管理端点 —— 那是**骨架数据线 + 骨架管理**的改动，不是 `M3` 的差集口径改动。而且即使改了，今天也没有任何一条采集路径会写入「没统计」，所以改完仍然到不了那一档。

**要动它，先答一个产品问题**：「近五年出现 0 次」与「我们还没统计过这个考点」在界面上是不是两句话？§二 说是，而骨架层今天说不是。⚪ 登记，不代填。

### 回写 5 · §9.4 的 `level` 今天恒为 `3`，因为骨架是两层

§9.2 / §9.4 按**三层**骨架写（`level` 1/2/3，第 3 层进分母）。实现里 `Syllabus` 是 **`Group → Node` 两层**，而 `GET /syllabus/nodes/{id}` 只解析得到叶子。

落法：`NodeDetailDto.level` 是一个常量 `3`（叶子就是进分母的那一层），代码里带 `ponytail:` 注记指明升级路径。**契约形状不变**，骨架长出中间层时改一处。

⚪ **不裁定**：骨架要不要长出第三层、什么时候长，归骨架数据线，本议题不代定。

### 回写 6 · §9.2 `GET /syllabus/tree` 的三层与 `archived` 区**本轮未实现**

理由与回写 5 同源：它要求响应里出现 `level` 1/2/3 三层嵌套与一个 `archived` 区，而这需要**骨架模型加一列 `level` 并允许中间层**——那是骨架数据线的改动，不是 `M3` 的差集口径改动，且本议题的范围表只给了 `kaodian-domain` 的公式与 `kaodian-app` 的四组端点。

**登记为落差，不代劳修**（见 §十五 新增行）。今天 `/syllabus/tree` 仍返回两层，`GroupDto` 已按 §9.2 改成 `touchedCount` / `untouchedCount` 由服务端给。

### 回写 7 · 判据 14 的 `grep` 分不开「盲区榜」与「盲区打开埋点」（**它从来就不是绿的**）
...
而 `blindspot` 这个词在这个仓库里**同时**是两样东西的名字：

| 出现处 | 是什么 | 该不该在 `domain` |
|---|---|---|
| `CoverageService#blindSpots` · `BlindspotOrder` · `BlindspotFilter` · `BlindspotCaliber` | **盲区榜**，差集口径本身 | ✅ **必须在** —— 它就是那条公式 |
| `blindspot_opened_event` · `BlindspotEventStore` · `NORTH_STAR_SURFACES` | **盲区打开埋点** | ❌ 必须在 `app` |

⚠️ **§十三 把这一条标成 ✅ 绿是一次误判**：`CoverageService.blindSpots` 在 `81a2c23` 上就存在，这条 `grep` 那天跑出来就是非零命中。判据没跑过，所以没人发现。

**判据改为**（对准埋点的名字，而不是「盲区」这个词）：

```bash
grep -rniE 'blindspot_?opened|BlindspotEvent|north_?star' \
  server/kaodian-domain/src/main/java/ ; echo "expect: 0 hits"
```

这条今天是**真绿**的：埋点的存储与端点全在 `server/kaodian-app/.../api/events/`。

⚠️ 别往这条正则里加 `localDate` 或 `surface`：`localDate` 会命中 `java.time.LocalDate`（备考档案在 `domain` 里合法地用它），`surface` 太泛。**判据的措辞本身要选得住** —— 一条会假阳性的判据，第一次被撞上时就会被人加个 `grep -v` 削掉，而削到最后它守不住任何东西。

### 回写 8 · 判据 9 的 `grep` 现在会命中三行，其中两行是对的

§3.2 写着 `grep -rn 'X-Config-Fallback' server/kaodian-app/src/main/java/` → 「expect: 恰好 1 个 Filter/Interceptor」。今天它命中三行：

1. `CaliberDeviationFilter.HEADER` —— 那个 Filter 本身 ✅
2. 同文件里的一行注释 ✅（`CLAUDE.md` §Delivery 那条「黑名单不得命中自己的合规注释」）
3. `ApiCorsConfig` 的 `allowedHeaders` 一行 ✅ —— **回写 3 要求的那一行**

**判据改为**：命中的**解析处**恰好一个（即 `Filter` / `Interceptor` 只有一个），CORS 那一行是它的前置条件而不是第二个解析处：

```bash
grep -rln 'X-Config-Fallback' server/kaodian-app/src/main/java/ ; echo "expect: 恰好 2 个文件 —— 那个 Filter + ApiCorsConfig"
```

### 回写 9 · ⚪ 「重算中」今天没有产生条件

§9.1 的 `recalculating` 与 §二 的「重算中四个 key 消失」已在 `SummaryDto.recalculating(...)` 上实现并有判据（`NullKeyOmissionTest`），但**没有任何调用方会构造它** —— 因为 §十六 待确认 3「覆盖度重算由什么触发、多久算完」还开着。

这是**有意的**：形状先定死，触发条件等那条待确认落地。⚪ 不裁定。
...
| 16 | `GET /syllabus/tree` 仍是**两层**，没有 `level` 字段、没有 `archived` 区 | `Syllabus` 是 `Group → Node`；`Syllabus.Node` 无 `level` 列 | 中 | 🔴 **骨架数据线**（加 `level` 列 + 允许中间层），不是 `M3` 的差集口径改动。见 §十四之二 回写 6 |
| 17 | `POST /agent/chat` 的 `context` 首轮绑定、`from` 闭集四值、`node-ref` 帧 | `AgentRequest` 只有 `(userId, sessionId, message, images)`；`AgentSession` 无 context 列 | 高 | 🔴 落在 **`kaodian-agent`**，而 `KUBI-103` 的范围表只给了 `domain` + `app`。归 stage 3 分配 |
| 18 | 🔴 `POST /agent/chat` **没有额度扣减挂钩点** —— §5.2 那条「一次会话 = 1 次，在首轮 `done{ok}` 扣」无处可挂 | 全仓 `quota｜ai_ask` 在生效代码零命中；`M7` 的额度账本（`KUBI-101`）还没开工 | 🔴 高 | **`M7` 先落账本，`M3` 再挂钩**。判据 `AgentQuotaTest` 在账本存在之前写不出来 —— 没有 `used`/`granted` 就断言不了「额度不许为负」 |
| 19 | 无 `GET /agent/suggestions` | 无此 controller | 高 | `M3` 后续（本轮未做，§5.3 的路径前缀黑名单已经把它挡在只读令牌之外） |
