...
| 6 | **`GET /billing/plans` 要令牌** | 它不在白名单五行里，走默认档 `scope=full`。**这是一次只改文档不改代码的裁定** | 白名单常量里没有 `billing/plans`（§7.2） | 🔴 待建（白名单本身归 `B0-4`） |
...
🔴 **它不在 §三 白名单五行里。** 五行是匿名端点的全集：`/auth/sms/send`、`/auth/sms/verify`、`/auth/wechat/login`、`/auth/agreements/current`、`/billing/notify/wxpay`。
...
| 3 | **`B0` 增量 2 的幂等列只写了三行** | `B0` §十五 增量 2：三行；`接口契约` §8.4 / §8.5 示例里逐字写着 `Idempotency-Key` | 幂等列应为**五行**（§8.1） | 本模块，落进 §十三 增量 1 |
...
### 增量 1 · §三 鉴权总表的「幂等」列，本模块占五行里的三行

> **`Idempotency-Key` 列今天要标的是五行，不是三行**：`POST /billing/orders`（外部账单）、`POST /billing/orders/{outTradeNo}/close`、`POST /billing/orders/{outTradeNo}/receipt/verify`（后两者的示例里本来就逐字写着这个头，见 §8.4 / §8.5）、`POST /ai/ask`、`DELETE /account`。
