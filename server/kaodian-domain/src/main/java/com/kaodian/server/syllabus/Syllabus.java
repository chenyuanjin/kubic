 ...
 * 再往下切会让粒度失控 —— docs/execution/阶段0至阶段2详细排期.md 甚至把「标到 500 题还在冒新考点」列为分类过细的诊断信号。
 * <p>
 ...
 * 不受真题汇编著作权保护 —— 依据 docs/data/INDEX.md:汇编著作权保护的是「选择与编排」,
 * 不延伸到底层事实。真题原文留在离线区,永不进入本仓库与线上库。
 * <p>
 * 考点命名为<b>自行归纳</b>,不沿用任何机构既有体系与措辞(R-07 / docs/decisions/实施路径.md §1.2)。
 * 这也是<b>没有「从机构导入考点体系」这类入口</b>的原因:批量导入的下一步一定是照抄。
 *
 * <h2>🔴 code 是主键,名字不是</h2>
 *
 * 行为层的每一条记录都挂在 {@link Node#code()} 上({@code Touch.nodeCode})。
 * 阶段 1 的任务是「骨架冷启动 + <b>人工校正命名</b>」(docs/decisions/实施路径.md §1.2)—— 也就是说,
