...
| **组装问 AI 的文本** | 🔴 端上生成，零网络请求。**没有端点就没有可以关掉的开关** | ✅ **已补（2026-09-03）**：`接口契约：签名与错误码全集` §6.1 就是「这里不该有端点」那一行 —— 缺的从来不是端点，是这句话没有契约承载 | ✅ `E-1`（§七）已闭合 |
| **AI 代发** | 先拿到模型名称与备案标识再发；流式；`done` 帧三条路径都要发出去 | ✅ **已补（2026-09-03）**：`接口契约：签名与错误码全集` §6.5 `POST /ai/ask` —— §6.7.1 那处自指一个不存在端点的问题随之消失 | ✅ `E-1`（§七）已闭合 |
...
⚠️ **本节各行的引用编号是 `E-1`–`E-10`**（此前只写了裸数字，导致 `E-1` 在 `导出与问AI` §十八 与
`文档规范与目录` §二 一共 5 处引用里是个悬空编号，2026-09-03 补）。
**`E-1` 在那 5 处指的是第 1、2 行的合称** —— 问 AI 那两条路径的契约缺口，历史用法，外部引用不必改。
所以 `E-1` 占两行，**`E-2` 空缺且不复用**（复用它会让「`E-1` 到底是哪两行」这个问题再问一遍），其余 `E-3`–`E-10` 一行一个。

| # | 缺口 | 缺的是哪一半 | 该谁定 |
|---|---|---|---|
| ✅ **`E-1`** | **免费复制路径在 `技术架构与接口契约` §六 没有对应行** | 产品侧已定：端上组装、零网络请求、永不移除。**契约侧空白** —— 「这条路径按设计不该有端点」这句话没有任何一行契约承载，于是它是一条靠记忆传递的约定 | ✅ **2026-09-03 技术侧已补**：`接口契约：签名与错误码全集` §6.1「这里不该有端点」—— 缺的那一行契约就是这一行 |
| ✅ **`E-1`** | **付费代发路径在 `技术架构与接口契约` §六 没有对应行** | 产品侧已定：发什么、什么时候扣、失败怎么办、六档错误要能区分。**契约侧空白** —— §6.7.1 已经引用了「代发端点」的内部扣减时机，而 §六 通读没有这一行 | ✅ **2026-09-03 技术侧已补**：`接口契约：签名与错误码全集` §6.5 `POST /ai/ask` |
| `E-3` | 导出的分块阈值与单文件大小上限（🚧 未实测） | 缺一个实测数：多大的量会让端上拼文件卡住、多大的文件会让 Web 端撑不住 | 技术侧按实测给 |
| `E-4` | 「很久没碰」的天数阈值与「最近的节奏」统计窗口（🚧） | 缺的是产品对「多久前」的一次分档口径，不该由实现顺手定 | **产品**，未定 |
| `E-5` | 记录正文要不要进导出，以及「导出我的全部数据」这句文案怎么改 | 产品裁定是不进；剩下的是那句「全部」怎么改，改哪一边要拍板 | **产品**，本域不改另一份规格的文案 |
| `E-6` | 模型返回越界内容时要不要做输出侧的词表闸 | `R-91` 未决：输出侧自检今天缺席，是补一道闸还是书面承认只靠数据面守 | **需人裁定**，本域不替它裁 |
| `E-7` | AI 代发的回答存不存 | 存了要回答「它算不算学习内容」；不存则换设备就没了 | **未定**，两条后果都要认 |
| `E-8` | 只读令牌的个数上限、名称长度上限、有效期（🚧） | 三个数都没有真源 | 技术侧 |
| `E-9` | 桌面壳算不算「Web 界面」 | 契约写死「在 Web 界面手动签发」，而桌面壳内嵌的就是同一份 web 产物 | **放宽它是安全边界的变更，产品不自行放宽** |
| `E-10` | 导出临时文件保留多久（🚧 占位 30 分钟） | 太短「再存一次」失效，太长一份含全部数据的文件在临时目录躺着 | 技术侧 |
| `E-11` | **预览文本的长度上限、接近上限的提示阈值、每一档最多列几个考点**（🚧） | 行为已定（超长提示缩小范围、🔴 不静默截断、段尾写明还有多少个没列出来），缺的是三个数。它取决于用户接的模型上下文 | **未定**，产品只能给一个保守值 |
| `E-12` | **代发的四个时间与两个长度**：帧间隔超时、整体超时、上下文过期、追问轮数上限、追问输入长度上限（🚧） | 首字超时可套全局请求超时，其余五个套不上 —— 流式的时间尺度和一次性请求不是一回事 | 技术侧。⚠️ 首字超时与整体超时必须是两个数 |

⚠️ **`E-11` / `E-12` 是 2026-09-03 补登记的**，不是新缺口：它们在 `导出与问AI` §二十一 一直是 `X-1` 与 `X-10`，
只是本节初次重排时漏了两行，于是 `U4.4` `U4.5` `U4.6` 的单元表里它们只能写成裸数字。补号即闭合这个漏洞。

下面三条是**本域自有、真源侧没有对应 `X-n`** 的缺口，按 `模块地图` §一 第 4 条带域前缀：

| # | 缺口 | 缺的是哪一半 | 该谁定 |
|---|---|---|---|
| `M4-G2` | 导出自检第 2 条里「单值长度的合理上界」（🚧） | 来源名称有上界，**章节路径没有**。自检要用一个数，而这个数没定 | 技术侧。定不下来自检只能退化成「只检字段名白名单」 |
| `M4-G3` | 导出元信息里版本标识的取值形态（🚧） | 形状已定，具体取值串未定稿 | 技术侧 |
| `M4-G4` | 分档阈值定下来之后由谁持有（端上写死 vs 服务端下发） | 端上写死改一次要发一次版；服务端下发则免费路径就有了一个网络依赖，**而零网络请求是它「没有开关」的前提** | **两条后果都要认**，本域不裁定 |

上述 `E-1`–`E-12` 与 `导出与问AI` §二十一 的 `X-1`–`X-11` 同源，本节是按「缺哪一半 / 该谁定」重排，**不是第二处登记**；
`M4-G2`–`M4-G4` 是本域自有的三条，真源侧没有对应行。**七份单元文档的 §五 一律引这两组编号，不再自己排裸数字。**
