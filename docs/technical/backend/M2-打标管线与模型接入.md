...
public record TagAttempt(
        long     recordId,     // 主键：一条记录只有一行「最近一次打标尝试」。B0-2：业务实体主键一律 long
...
public record RecordTag(
        long      id,           // B0-2：业务实体主键一律 long
        long      recordId,     // B0-2：同上，指向 Touch.id
...
| `UNAUTHORIZED` / `TOKEN_EXPIRED` / `ACCOUNT_DEACTIVATED` | 401 | **三档**（不是两档），拆法见 `B0` §5.3 与 `接口契约` §1.2。三档都由 `B0-4` 的过滤器抛，**M2 域内不主动抛** | §10.2 / §10.3 已有 |
...
**裁定：不是三处。** 理由与 `B0` 给 `POST /ai/ask` 的那条逐字相同：外部模型调用 = 按次外部账单，重放一次就是扣两次额度。

⚠️ **2026-09-03 更正（KUBI-89 审核轮）：本文上一版判「四处」，那也是数漏的。**
四份模块设计各数出一个数（`B0` 三 / `M2` 四 / `M4` 五 / `M7` 五），stage 3 已取**并集**并落进 `接口契约` §3.3 —— **那张表是唯一真源，本文不再自己数**。`suggest` 在那张表上。
🔴 原来那句担心仍然成立，而且正是这么发生的：「哪些端点要带幂等键」被四份文档各存了一份，**对不上的那一处会扣两次钱**。所以终局不是「把数字改对」，是**只留一处能数**。
...
### 增量 1 · §三 鉴权总表「幂等」列，本模块认领其中一行

⚠️ **2026-09-03 更正（KUBI-89 审核轮）：本增量上一版的标题是「标的是四行不是三行」。**
四份模块设计各数出一个数，stage 3 已取**并集**并落进 `接口契约` §3.3（唯一真源），本增量的行数断言已被它覆盖。
🔴 **本段不再复述行数** —— 照上一版合并会在 §三 里同时出现两个互相矛盾的行数断言。

> **`POST /records/{id}/tags/suggest` 在 `接口契约` §3.3 那张表上**，§4.1 正文已逐字写死它必带 `Idempotency-Key`。
> 理由与 `POST /ai/ask` 逐字相同：**外部模型调用 = 按次外部账单，重放一次就是扣两次额度。**
> 保留期由本模块定为 **24 小时**（已随 §3.3 落地）。
...
### 增量 8 · ⚠️ 落地实读（KUBI-102，2026-09-04）：本文 §10.1 那三个 `long` 与 `B0` 落的代码对不上

> **实读 `B0`（`KUBI-106` 分支 `7abdfa1`）的 `RecordTag`**：`id` / `recordId` / `nodeCode` 仍是 `String`，**只有 `userId` 收成了 `long`**。
> 本文 §10.1 按「`B0-2`：业务实体主键一律 `long`」写成 `long id, long recordId`，**那是设计当时的预期，不是 `B0` 交付出来的形状**。
>
> **裁定：跟着代码走，`TagAttempt.recordId` 用 `String`。** 理由是 `TagAttempt.recordId` 必须与 `RecordTag.recordId` / `Touch.id` **同一个值域** —— 三者对不上时，队列会捞不到自己那一行，而它不会报错。
> 收 `long` 是一次跨 `M1`/`M3`/`M4` 的存量迁移，不在本模块的范围里；与 `M2-G2`（节点标识值域）是同一类问题，**一起裁，不各裁一半**。

### 增量 9 · 🔴 `SERVER_ERROR` 的状态码：§4.1 与 §10.2 自己对不上

> **`接口契约` §4.1 写「服务不可用 / 超时 → `502` / `504` `SERVER_ERROR`」，而同一份文档 §10.2 那张表把 `SERVER_ERROR` 钉在 `500`。**
> `B0` 的 `ErrorCode` 枚举照 §10.2 落了地（`SERVER_ERROR(500)`，状态挂在枚举上、一处决定），
> 而 `ErrorCodeContractTest` 双向比对的是 §十那张表 —— **所以今天代码里拿不到一个 `502` 的 `SERVER_ERROR`，除非改横切件。**
>
> **本模块不改横切件，也不用已废弃的构造器自己挑一个状态**（那正是 `B0` 收掉的那件事）。
> 落地取 **`RECOGNIZER_UNAVAILABLE`（`503`）**，理由不是将就：今天识别不可用**确实发生在发出外部调用之前**（真实厂商实现还不存在，`StubVisionTagger` 是本地判定），
> 而 §十一 给这一档写的正是「识别服务本地判定不可用（未发出外部调用）→ `503`」。
>
> 🔴 **真实厂商实现落地那天这一格必须重判**：那时「上游 502 / 超时 504」与「本地判定 503」是两件事，而 §10.2 让前者无处可去。
> **该定的是 §4.1 与 §10.2 谁让步**，归 stage 3 收口，本模块不代裁。

### 增量 10 · ⚠️ §2.5 那条 grep 判据今天在 `tagging` 之外是红的，而且不是本模块弄红的

> 判据原文：`grep -rniE 'quota|billing|order|额度' server/kaodian-domain/src/main/java` 零命中。
> **实读命中四个文件**：`TouchKind`（`consumesAiQuota` 一个布尔标志 + javadoc）、`CaptureService`（javadoc）、`AsrClient` / `StubAsrClient`（javadoc）。**全部是 `v1` 上早就有的**。
>
> 它们**不是一条依赖边**：没有一处去问「还剩多少」，`TouchKind.consumesAiQuota()` 是「手动三种永不消耗」这条产品结论的落点。
> 但判据写成「零命中」就等于它**从落地第一天起就是红的**，而一条一直红的判据等于没有判据。
>
> **落地取的形态**：`TaggingBoundaryTest#commerceNeverLeaksIntoTagging` 钉两件事 —— ① `tagging` 包一处都不许有；② `domain` 其余部分的命中**就是已知那四个文件，多一个就红**。
> 判据原文该怎么改（收窄到 `tagging`，还是把那四处一起清掉）归 stage 3 定。
> 顺带：原判据里的 `order` 会命中 `ordered` / `orderBy` / `recorder`，**这个词单独拿掉**。
...
| `M2-G3` **`remaining` 今天恒为空对象** | §9.1 要求响应带 `remaining`，而它的内部形状归 `M7`。`M7` 落地之前本模块**一个键都不自己发明** —— 发明一个「猜的形状」，`M7` 落地那天前端要改两次 | `M7` |
| `M2-G4` **自动重试没有驱动方** | §五 的队列状态机（进队条件、退避、上限、丢最旧）已落地，但**没有任何东西在把队列捞起来**。今天捞了也没用：事后补标手里一份素材都没有（`M2-G1`），重试的结局只会是同一个 `NO_MATERIAL`。造一个每分钟得到同一个结论的定时任务，是把「这条路走不通」藏起来 | 随 `M2-G1` 一起 |
| `M2-G5` **`TS-05` / `TS-06` 对存量记录不可达** | `Touch` 的构造器要求 `nodeCode` 非空（「必须挂到一个考点上」），于是每条记录都自带一条**已确认的主标签** → 状态恒为 `TS-07`。未分类计数因此**结构上恒为 0**，除非用户先把全部标签丢掉。这不是 `TagState` 算错了，是「记录可以先落地、后打标」这条路今天在 `Touch` 上就不成立 —— 与 `M2-G1` 同源 | 产品 + `M1` |
