...
- **The trunk is `v1`** — decided by a human on 2026-08-30 (`00` §2.4-①). Branch `KUBI-<n>-<slug>` off `origin/v1`, target PRs at `v1`, and write baselines as `基线:origin/v1 @ <SHA7>,fetch 于 <时间>`. `main` was fast-forwarded to `v1` on 2026-08-30, so both names now point at the same commit — **but `main` is a mirror, not a second trunk: never commit to it.** It was a strict ancestor of `v1` (`origin/v1..origin/main` was empty), so merging `main` into `v1` was a literal `Already up to date`; the only file it had that `v1` lacked was `.DS_Store`. ⚠️ The sync is **one-off** — the next push to `v1` leaves `main` behind again, and that is not an incident. The durable fix is a repo admin repointing `origin/HEAD` / the GitHub default branch at `v1`; see `00` §2.6 E1.
