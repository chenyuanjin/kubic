...
| # | 共享的东西 | 内容 |
|---|---|---|
| **S-1** | **状态名全集** | 十态取自 `模块地图` §4.3，本域一个都不自造、不加同义变体。徽标文字同样以那张表为准。⚠️ **`模块地图` §4.3 只是转录，真源是 `打标与未分类` §4.1** —— 两者不一致时以真源为准并回头改转录（2026-09-03 就修过一次：那张表把 `TS-08` 写成计覆盖度，与真源相反） |
...
| ✅ ~~`T-6` **搜索结果条数上限**~~ | **已关闭（2026-09-03）**：裁定**不建搜索端点**，搜索改为端内过滤，条数上限这个问题随之消失。真源 `U2.4` §三 / §五 / §7.3 已改齐 | 已闭合。理由见 `接口契约：签名与错误码全集` §6.4 |
