#!/usr/bin/env bash
# 闸 1(机器闸)判决回写 —— docs/14 §9.2 五键契约 + §9.6 属性
#
...
#   通道污染,而这条通道今天连通不通都还没验证过(docs/14 §八:GitHub Actions
...
# ══════════════ 1. 议题号 ══════════════
# docs/14 §9.3:分支形如 KUBI-12-offline-queue。Multica 的 PR ↔ 议题关联就是靠
...
if ! curl -sS -o /dev/null --max-time "$PROBE_TIMEOUT" "$SERVER_URL" >/dev/null 2>&1; then
  ...
  say "docs/14 §八 登记的待确认项:GitHub Actions 出口能否访问该端口。打不通就退回本地 runner。"
  exit 0
fi
