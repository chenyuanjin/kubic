...
> **状态：活跃。** 三条计算规则任一变化、断言形态变化、`blindspot_opened` 的口径变化、`S-ASK` 的四个入口增删、`S-BLIND` 统计口径行的档数变化，本文同一次改动内更新。
> **上一层**：[模块地图](../INDEX.md)
...
| [`U3.8` 我要考的那一场](U3.8-我要考的那一场.md) | 备考档案两个字段：考哪一场、什么时候考；**第一次进 `S-ASK` 前**收一次（可跳过）、`S-SET` 可改 | 让差值**贴住用户那一场考试**。⚠️ 今天只服务 `U3.7` 的上下文，**不改排序** |
...
**2026-09-03 新增十一条（`U3.7` `U3.8` 落盘时开的，其中三条当天即闭），按 `模块地图` §1.1 规则 4 一律带域前缀：**

| # | 缺口 | 该谁定 | 展开在 |
|---|---|---|---|
| ✅ `M3-G1` | ~~备考档案收集屏占掉「登录之后 2 次点击」里的一次~~ **已闭合**：收集时点挪到**第一次进 `S-ASK`**，`首次使用与权限申请` §1.2 那两个数一个不改 | —— | `U3.8` §五 |
| ✅ `M3-G2` | ~~「其他考试」这一档是界面推出来的映射~~ **已作废**：产品裁定档案照存全闭集 | —— | `U3.8` §五 |
| ✅ `M3-G2b` | ~~契约 `:1941` 的 `EXAM_TYPE_UNSUPPORTED` 没写落在哪个端点~~ **已闭合**：`8855bec` §12.9.4 挪出现役码表 | —— | `U3.8` §五 |
| ✅ `M3-G10` | ~~契约 `:1568` 的 `S-BLIND` 上下文里还有 `province?`~~ **已闭合**：`8855bec` 已删，锚点只剩三个 | —— | `U3.7` §五 |
| ✅ `M3-G3` | ~~`S-ASK` 没有逐屏规格；文案真源住在契约里~~ **已闭合**：`specs/问一下.md` §二落盘，**十五条**建议问法 + 五条上下文摘要 | —— | `U3.7` §五 |
...
| ✅ `M3-G8` | ~~`S-ASK` 是不是第十四屏，`specs/` 侧没有跟上~~ **已闭合**：`specs/` 与 `modules/` 侧 34 处屏数断言一次扫齐（规格 `6cb5490`）。⚠️ 余下三处都是**有主人的**：技术侧两处（`技术 INDEX:816` / `接口契约:1478`）归技术经理；`无障碍与多端适配` §8.2 逐屏焦点链按用户 2026-09-02 裁定不加厚（`Q-2`）；`端到端验收用例集` §5.1 已按十四屏改、`S-ASK` 那行标 ❌ —— **分母动了分子没动，是明着欠的**（`Q-4`） | —— | `U3.7` §五 |
| ✅ `M3-G11` | ~~发图那一格的第二句没有真源~~ **已闭合**：`R-89` 那条上屏，但**并进留存那一句**不另起（`问一下.md` §2.4） | —— | `U3.7` §五 |
| ✅ `M3-G12` | ~~档案屏主按钮那一句没有真源~~ **已闭合**：主按钮是「好了」，🔴 不用「记」（那是北极星的动词） | —— | `U3.8` §五 |
