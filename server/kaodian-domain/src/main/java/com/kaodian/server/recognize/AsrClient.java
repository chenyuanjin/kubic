 ...
 *
 * docs/识别链路 坑三:选型推荐的视觉模型标注为<b>实验版本,官方未给出任何稳定性承诺、
 ...
 * <p>
 * docs/技术架构 §2.1 把这一层画成整个后端<b>唯一允许出现外部模型调用的地方</b>。
 * 别处出现 HTTP 调模型,就是绕开了这个切换点。
 *
 * <h2>🔴 签名上没有 URL、没有 fileId、没有任何存储路径</h2>
 *
 * 入参是音频<b>字节</b>,一次过、用完即弃。docs/技术架构 §八:服务端不写磁盘、不进对象存储。
 ...
 * docs/技术架构 §3.1 的原话是「<b>这两个接口的签名是本文档里唯一值得现在就写下来的代码</b>」——
...
public interface AsrClient {
     ...
     * @throws RecognitionUnavailableException 服务不可用/未配置。<b>失败就抛,不返回半成品</b>(docs/技术架构 §3.1)
     */
