 ...
 * 逐条记录一次要多少条。
 *
 * 🔴 100 是服务端 `Cursor.MAX_LIMIT` 的值(参数名就叫 `limit`,默认 20)。
 * 上一版写的是 200 —— 那是旧的上限,服务端已按 `接口契约` §1.4 收到 `1..100`,
 * <b>再传 200 会直接 `400 INVALID_LIMIT`</b>。
 * 要满额是因为每个考点的做题数由这批记录求和(树接口不返回做题数),
 * <b>取不满就得整体显示「—」</b> —— 见 derive.ts 的截断闸门。
 * <p>
 * 服务端的 cursor 分页已经有了(`nextCursor`),但这里<b>仍然只拉第一页</b>:
 * 一屏要么拿到全量、要么诚实地说「算不准」,在后台连翻几页拼一份「看起来是全量」的和,
 * 恰好是这个闸门要挡的那件事。真到超过 200 条的量级,该改的是这一屏怎么承认自己算不准,
 * 不是在这里多发几个请求。
 *
 * ⚠ 单模块 18 个考点、种子 8 条记录,100 条今天绰绰有余;真到超过 100 条的量级,
 * 闸门会开始把这一屏判成「算不准」—— 那是它该做的事,不是一个要绕开的限制。
 */
 ...
 *
 * 失败时<b>如实报错</b>。docs/execution/INDEX.md §1.3.7 的「记录动作永不失败」说的是识别挂了不能连累落库,
