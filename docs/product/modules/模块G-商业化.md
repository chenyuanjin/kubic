...
>
> **不是**定价与单位经济测算（在 [`商业化设计`](../商业化与额度设计.md)，含额度表、毛利、敏感度、支付合规）、
> **不是**订单与扣减的后端时序（在 [`后端详设`](../../technical/后端系统设计与组件接入.md) §1.10 §1.11）、**不是**接口契约（在 [`技术架构`](../../technical/INDEX.md) §6.6 §6.7）、
> **不是**界面稿（在设计侧）。本文写到「状态 + 转移条件 + 失败落点」为止，
> **不定字段、不定接口、不写任何一个价格或额度数字** —— 那些数字只有 `商业化设计` 一处，本文只给指针。
>
> **基线：`origin/v1` @ `afab5a7`，fetch 于 2026-08-31T05:20Z。**
> **状态：冻结待触发。** 与 `商业化设计` 同一档 —— 关卡 2 之前本模块的正确状态是**不存在**（§2.6）。
>
> ⚠️ **本文服务关卡 2 之后，不服务关卡 0。** 写它不产生 `1.1.4` 那两个每日数字，一个都不产生。
> 而且 `商业化设计` §十 已经点名：**商业化文档是「注意力流向能动手做的部分」的顶级高发区**，
...
**代价要认：产品最有价值的部分永远免费，收费的是最没有差异化的部分 —— 调 API 谁都会**（`商业化设计` §3.3 原话）。
...
**归到失败是用户明明扣了款、界面说没付上，他会再付一次 —— 那是产品主动诱导重复付款**（`后端详设` §1.10）。
...
`后端详设` §1.11 给了三种写法与各自后果，并指出**规则属于用户协议、架构定不了**，待律师稿。
...
⚠️ **先读 `商业化设计` §十**：商业化文档是 `思考模式` §盲区二 的顶级高发区，
...
|---|---|---|---|
| 额度 | 耗尽时降级为手动记录，**不阻断记录动作** | 扣减**没有独立端点**，只发生在 AI 端点内部；**失败不扣** | `技术架构` §6.7 🔴 |
| 额度展示 | 把额度换算成**用量**呈现，不只呈现次数 | 服务端返回剩余量 | `商业化设计` §一 · `R-48` |
| 档位与定价 | 档位与额度由服务端返回，**前端不硬编码** | 下单只收档位码，**金额由服务端定** | `技术架构` §6.6 🔴 |
| 盲区诊断 | 🔴 **不存在付费入口** | 无计费挂钩 | `商业化设计` §2.1 🔴 |
| 导出 | 🔴 **不限次数、无水印、无删减** | 无计费挂钩 | `商业化设计` §2.2 🔴 |
| AI 代发 | 额度用尽降级为免费复制；**回答区标注来源模型** | 代发端点内部扣减；**答案不落库** | `商业化设计` §五 · §六 G-2 |
| 订单 · 确认中 | **不给「重新支付」**，给「再查一次」+「先去看盲区」 | 三路幂等，重复到达不重复发放 | `后端详设` §1.10 |
| 续费 | 把「不自动扣费、到期就停」**写在购买页当卖点**，不藏进协议 | 续费是延长到期时间，不新建 | `商业化设计` §6.3 |
...
| 边界 | 落在本模块上是什么形态 | 真源 |
|---|---|---|
| 🔴 **盲区诊断永不上锁** | 结构上不存在付费入口。上锁会污染关卡 2 判据，而**判据被污染的结果不是数字变差，是数字变得没有意义** | `商业化设计` §2.1 |
| 🔴 **导出永不上锁** | 无删减、无水印、不限次数 —— 字面意思 | `商业化设计` §2.2 |
| 🔴 **不引导至外部支付** | 小程序自带完整支付路径；**不做分端差价** —— 差价本身就是一条指向外部支付的箭头 | `R-36` · `商业化设计` §6.2 |
| 🔴 **不做教研** | 代发只组装上下文 + 代调 + 原样透出，**不摘要、不评分、不加工**；答案不进覆盖度 | `商业化设计` §5.2 |
| 🔴 **不存内容** | 代发的答案不落库 —— 库里**没有字段能装下一段答案** | `商业化设计` §5.4 · `R-06` |
| ⛔ **不做积分 / 打卡 / 徽章** | 会员权益里不出现任何以打开次数为奖励对象的东西 | `技术架构` §十 |
...
**付费验证本身在关卡 3 之后**（`实施路径` 阶段 4 / `执行清单` `L-D` 档 / `总路线图` §2.4）。
...
| G-2 | **代发的免费/付费两条路径都没有接口契约行** 🔴 | 与 `模块E` §六 `E-1` 是**同一个缺口**，不是第二个 —— 它挂在 E 出口层，本模块只是它的计费侧 | 技术同事补契约。**本文不重复登记，只指过去** |
| G-3 | **只有一个付费档 → 没有价格锚点** | `R-48` 已登记。缓解只有一条且很弱：**购买页用「用量」当锚点顶替「价格」锚点** | 产品可执行这一条缓解；**加不加第二档要等真实用量分布，不是现在猜** |

### 一句必须留在最后的话

`商业化设计` §十 已经写过，本文照抄不改：**这里面没有一个数字来自真实用户。**
