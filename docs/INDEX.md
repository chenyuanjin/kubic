...
| **展开** | [`用户侧功能规格`](product/specs/INDEX.md) **十二份**逐屏规格 · [`模块地图`](product/modules/INDEX.md) 的**九个域 51 个能力单元**（`M0`–`M8`；旧的「模块 A–G」七域口径 2026-09-02 已废） · [`商业化与额度设计`](product/商业化与额度设计.md) 商业化与额度 · ⛔ **H 组无展开文档**（原 [`产品路线图与用户共创`](misc/产品路线图与用户共创-已作废.md) 已作废） | [`接口契约：签名与错误码全集`](technical/接口契约-签名与错误码全集.md) 签名与错误码 · [`后端系统设计与组件接入`](technical/后端系统设计与组件接入.md) 后端详设 · [`多端选型与端矩阵`](technical/多端选型与端矩阵.md) 端矩阵 · [`Agent 框架与能力边界`](misc/Agent框架与能力边界.md) Agent 框架（收纳在 `misc/`） |
