...
> **这份文档回答一件事:这个产品由哪些模块组成,每个模块在「减法」的哪一端,以及它的前端行为与后端契约意图各是什么。**
>
...
>
> **状态:活跃。** 产品模块增删、或某个模块在减法里的归属被人裁定,本文同一次改动内更新。
>
> **本文之下有一层:** 脑图在 [`产品模块脑图`](产品模块脑图.md)(markmap 格式,可折叠);A–G 七组各有一份展开文档
> [`模块 A · 骨架层`](modules/模块A-骨架层.md)–[`模块 G · 商业化`](modules/模块G-商业化.md);用户看得见的功能逐屏规格在 [`specs/`](specs/INDEX.md)。
> **本文只回答「有哪些模块、各在减法的哪一端」**;「模块自己怎么运转」在各模块文档里,「屏幕长什么样、点下去发生什么」在 `specs/`。
> **H 组当前没有展开文档,X 不单独成文** —— [`产品路线图与用户共创`](../misc/产品路线图与用户共创-已作废.md) 已于 2026-08-31 作废(裁定人 chenyj),逐组落点与理由在 §九。
>
> **这份文档不产生任何一个关卡数字。** 关卡要的是「自用两周、每天记两个数」(`实施路径` §阶段0 / `总路线图 · 脑图与父子待办` `1.1.4`),
...
| **待办编号取自 `总路线图 · 脑图与父子待办` 待办树** | 编号只是排期指针,不代表功能的有无 —— 表里每一个模块都是完整 App 的一部分。先做哪块后做哪块由项目经理在排期文档里定,本文不排期、不判先后 |
...
**`X` 不是一个功能模块,是这张图的负空间。** 它不属于任何一个模块,它约束所有模块 —— 展开见 §六。

---

## 三、模块总表:每一格都要能填,填不出就停在待裁定

**下表里每一个模块都是完整 App 的一部分,不带阶段前提**(2026-09-02 裁定)。待办编号**取自 `总路线图 · 脑图与父子待办` 待办树**,只作排期指针 —— 先做哪块后做哪块由项目经理定,本文不判。

**本文之下这一层一共八份:** 脑图 [`产品模块脑图`](产品模块脑图.md)(markmap,只答「有什么、挂在谁下面」),
加 **A–G 七组各一份展开文档**,写「模块自己怎么运转」:
[`模块 A · 骨架层`](modules/模块A-骨架层.md) · [`模块 B · 采集层`](modules/模块B-采集层.md) · [`模块 C · 打标层`](modules/模块C-打标层.md) · [`模块 D · 差集层`](modules/模块D-差集层.md) · [`模块 E · 出口层`](modules/模块E-出口层.md) · [`模块 F · 账号与端`](modules/模块F-账号与端.md) · [`模块 G · 商业化`](modules/模块G-商业化.md)。
**H 组当前没有展开文档**([`产品路线图与用户共创`](../misc/产品路线图与用户共创-已作废.md) 已作废)**,X 不单独成文,理由在 §九。**
下表不变,它仍然是唯一的模块总表。

| 模块 | 在减法的哪一端 | `总路线图 · 脑图与父子待办` 编号 | 真源 |
|---|---|---|---|
| **A 骨架层** | 被减数 | `1.2.1`–`1.2.3` `1.2.7` | `实施路径` §1.2、`基础数据 · 抓取范围与渠道`(原料来源) |
| A · 节点归档 | 被减数 | `R-49` | `后端系统设计与组件接入` §1.4 |
| **B 采集层 · 四入口** | 减数入口 | `1.1.1` `1.1.2` | `后端系统设计与组件接入` §1.2 |
| B · 离线队列 | 减数入口 | `1.1.2b` | `总路线图 · 脑图与父子待办` `1.1.2b` |
| B · 原图留存与到期归档 | 减数入口 | `1.1.3` 🔴`R-04` | `技术架构与接口契约` §八、`原图存储：判据层与存储层` |
| B · 记录动作永不失败 | 减数入口 | `1.3.7.1` | `后端系统设计与组件接入` §1.1 §1.5 |
| **C 打标层** | 减数成形 | `1.2.5` | `后端系统设计与组件接入` §1.3、`识别链路选型 · ASR 与图像识别` 坑一 |
| C · 手动挂载与未分类兜底 | 减数成形 | `1.2.5.1.5` | `技术架构与接口契约` §6.3 |
| **D 差集层 · 覆盖度与盲区** | 减法本身 | `1.2.8` | `后端系统设计与组件接入` §1.4 |
| D · 正向断言 我已掌握 | 减法本身 | `技术架构与接口契约` §6.4 | `技术架构与接口契约` §6.4 |
| D · **反向断言 我卡住了** | 减法本身 | `1.4.5` | 缺口,见 §七-② |
| **E 出口 · 盲区清单与 Top N** | 出口 | `1.2.8.4` | `技术架构与接口契约` §6.4 |
| E · 导出 三格式 | 出口 | `1.3.6` 🔴 | `商业化与额度设计` §2.2 |
| E · **问 AI 时带上的东西** | 出口 | 未收录 | 缺口,见 §七-① |
| E · AI 代发提问 | 出口 | `商业化与额度设计` §五 | 缺口,见 §七-① |
| E · MCP / CLI 只读 | 出口 | `1.4.2` | `技术架构与接口契约` §6.5 |
| **F 账号体系** | 支撑 | `1.3.1` | `后端系统设计与组件接入` §1.6–§1.10 |
| F · 微信授权与打通 | 支撑 | `1.4.1` | `后端系统设计与组件接入` §6.1 |
| F · 端矩阵 · Web/H5 | 支撑 | `1.3.2` | `多端选型与端矩阵`(分支) |
| F · 端矩阵 · 桌面 / iOS / Android | 支撑 | 不在 `总路线图 · 脑图与父子待办` 产品轨 | `多端选型与端矩阵` §一,自述贡献为零 |
| F · 端矩阵 · 微信小程序 | 支撑 | `执行清单` `L-C` 档 | `多端选型与端矩阵` §3.6 |
| **G 商业化** | 支撑 | `商业化与额度设计` §零 | `商业化与额度设计` §一 §二 |
| G · 退款 | 支撑 | 未定义 | 缺口,见 §七-③ |
| **H 对外路线图与共创** | 支撑 | 无 | ⛔ 原真源 `产品路线图与用户共创` 已作废(2026-08-31),**这一组当前没有真源**。重建时另领新简称,不改那份作废文档(`文档规范与目录` §六 第 10 条) |
| **X 边界层** | — | 见 §六 | `项目现状与决策记录` §2、CLAUDE.md |
...
| **这张表里填不满的那几格** | **4**:桌面/iOS/Android 端(不在产品轨)、问 AI 带上的东西(§七-①)、退款(§七-③)、对外路线图(真源已作废) |

**数它的命令(别用眼睛数):** `awk '/^\| 模块 \|/,/^### 3.1/' docs/product/INDEX.md | grep -c '^| '`

**这四格里,「端矩阵 · 桌面 / iOS / Android」是唯一一格已经有实测产出的。**
`多端选型与端矩阵` §一 记着 macOS 实测可装可开、iOS 模拟器跑通、Android 代码侧已验 —— 三个端,而 `总路线图 · 脑图与父子待办` 产品轨里
只列了一个 Web/PWA(`1.3.2`),`1.3.2.1` 甚至明写「不做小程序,行政流程不进关键路径」。
...
**这不是要现在补契约。** `总路线图 · 脑图与父子待办` `1.4.5` 只列节点、不排期,现在动它就是 `R-85` 的又一个实例。
...
| **B 采集层** | [`模块 B · 采集层`](modules/模块B-采集层.md) | 四个记录入口都在这一组 |
...
### 9.2 模块文档展开到哪一步,九组一视同仁

**规则:**

> **模块文档只把「真源里已经定过的东西」展开成「状态 + 转移条件 + 失败落点」,不论它在减法的哪一端;
> 它不排期、不新定义任何东西。**
...
**代价也说清楚:这七份展开文档一个关卡数字都不产生**,每一份的头注都写着这个意思。

---

## 十、模块 → 屏幕规格对照

**模块文档回答「为什么这么设计」,[`用户侧功能规格`](specs/INDEX.md) 回答「它长什么样」。** 一个模块可能落在多屏,一屏也可能横跨两个模块。

| 模块组 | 屏幕 | 规格 |
|---|---|---|
| **B 采集层** | `S-HOME` `S-REC` `S-TL` | [记一次学习](specs/记一次学习.md) |
| **C 打标层** | `S-TAG` + 卡片内状态 | [打标与未分类](specs/打标与未分类.md) |
| **A 骨架层 · D 差集层** | `S-BLIND` `S-NODE` | [看盲区](specs/看盲区.md) |
| **E 出口层** | `S-EXP` | [导出与问AI](specs/导出与问AI.md) |
| **F 账号与端** | `S-LOGIN` `S-ACC` `S-MERGE` | [登录与账号](specs/登录与账号.md) |
| **G 商业化** | `S-BILL` + 各屏受限态 | [额度与购买](specs/额度与购买.md) |
| **X 边界层** | `S-SET` `S-RAW` | [设置与原图](specs/设置与原图.md) —— **边界第一次有了用户可读的落点** |
| **骨架原料(不是模块)** | **没有屏幕** | [骨架数据采集流水线](../data/骨架数据采集流水线.md) · 纯后端线 |

⚠️ **`X 边界层` 在规格里第一次变成用户看得见的东西**([设置与原图](specs/设置与原图.md) §二「这个产品不做什么」)。
在此之前它只是负空间 —— 只写在文档里的边界,用户一个字都读不到。
