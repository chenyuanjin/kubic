 ...
 *
 * <h2>前缀是 {@code /api/v1},与契约 §三 那张表逐字同一份</h2>
 *
 * {@code B0} §16.1 第 2 条曾把它写成「按临时前缀 {@code /api} 生效,迁移时点由项目经理排」;
 * <b>KUBI-107 已把 {@code /api/**} → {@code /api/v1/**} 三边一次迁完</b>(server 的
 * {@code @RequestMapping}、{@code web/} 的 {@code API_BASE}、{@code shell/} 的代理),
 * 于是这里按 B0 自己写的「改 {@link #PREFIX} 一行」跟上。
 * <p>
 * 这里<b>只写一个前缀</b>。🔴 <b>不许两个前缀都写一遍</b> —— 那会让
 * 「匿名入口的全集在这一处数得清」当场失效,而那正是白名单存在的全部理由。
 * {@code ApiAuthWhitelistContractTest} 盯着它与契约 §三 那张表继续对齐。
 */
@Component
public class ApiAuthFilter extends OncePerRequestFilter {

    /**
     * 🔴 生效前缀。见类注释 —— KUBI-107 迁完 {@code /api/v1} 之后这里跟着改了一次
     * (B0 §16.1 第 2 条自己写的那一行)。
     *
