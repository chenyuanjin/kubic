 ...
 *
 * docs/13 §1.7:契约里没有 {@code /auth/register}。
...
public class AccountService {
    ...
    /**
     * 微信通道登录 —— <b>关卡 2 后</b>。docs/10 §7.1 那张场景表的落地。
     *
     ...
     *
     * 这说明分裂已经发生过了。绝不自动合并 —— docs/10 §7.1 的理由是
     ...
     *
     * 不是随便挑的:docs/10 §7.2 已定<b>阶段 2 只做手机号,微信在关卡 2 后</b>。
     * 所以微信那个账号必然更晚建、更可能是个刚建的空号,而记录大概率在手机号那边。
     * 登错一边的代价是用户打开就看见一个空白的盲区页 —— 而那正是这个产品的首屏。
     *
     * <p>⚠ 这条路径<b>契约里没有</b>(docs/10 §6.1 那张表写于手机号快速验证未纳入考虑时)。
     * 它是新增的,不是对既有条目的改写。
     *
    ...
    /**
     * 已登录账号绑微信 —— docs/10 §7.1:<b>最顺的那条路径,产品应主动引导走这条</b>。
     *
     ...
     * 在今天的数据模型里<b>无处可搬</b>。
     * <p>
     * 把行为层改成多租户是一次独立的、比本模块更大的改动(docs/10 §5.2 的 {@code record_event.user_id}),
     * 这里不顺手做半个 —— 半个的后果是「有些记录带 user_id 有些不带」,
     * 而那会让覆盖率的分子在某些账号上凭空少一截。
     * <p>
     * 合并端点因此与微信登录一起被关在<b>关卡 2 后</b>的开关后面(docs/10 §6.1)。
     */
     ...
     *
     * <p>🔴 服务端数据的<b>硬删时点本层不定</b>。docs/10 §6.1 明确把它留给 {@code L-A5} 的律师稿:
