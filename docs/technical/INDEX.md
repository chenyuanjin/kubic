...
>
> **一个例外,必须自己点破:§5.5 / §5.6 / §6.6 / §6.7 这一整块商业化内容跑在阶段 2 后面。** 留它在这里只有两个理由:一是它记的是**不建什么**(不建代扣字段、不建加油包表、不建 prompt/答案列、不给扣减开端点),而「不建」写晚了就已经建上了;二是 `商业化与额度设计` `R-37`——额度只要提前生效,阶段 0 的「日均 ≥3 条」必然不过。**它是一条边界线,不是一份开工图。** 判据仍在 `实施路径`,排期仍在 `执行清单`/`总路线图 · 脑图与父子待办`,本文一件都不重排,**也不因为写完了就往前挪一格。**
>
...
| §七 | 鉴权与会话 | 阶段 2 `1.3.1`;微信通道在阶段 2 后 | 短信签名报备 `2.1.5` **现在就能办** |
...
> **商业化相关的 §5.5 / §5.6 / §6.6 / §6.7 最早落地阶段是「阶段 2 后」,比本表其余各节都晚。** 设计依据在 `商业化与额度设计`,本文只写它在数据模型与接口契约里的位置。🔴 **这几张表在阶段 2 之前一张都不建** —— `商业化与额度设计` `R-37`:额度提前生效会让阶段 0 的「日均 ≥3 条」判据必然不过。
...
**最早落地阶段:阶段 1 `1.2.4` 数据层落库。阶段 0 是本地文件夹 + 纯文本(`阶段 0 至阶段 2 · 详细排期` §技术选型),不动。**
...
| **桌面客户端** | 先用 PWA 安装;真需要本地能力时套 Tauri 2 壳 | 复盘场景(考点树 + 盲区清单) | **阶段 2 后**,见 4.4 |
| **微信小程序** | Taro 4(React 语法),**只复用逻辑层不复用 UI** | `L-C6`:只做采集入口,分析跳网页端 | **阶段 2 后**(`2.3` C 档) |
...
| POST | `/auth/wechat/login` | 小程序 code 换会话 | 阶段 2 后。命中已存在 unionid → 直接登录 | 阶段 2后 |
| POST | `/auth/bind/phone` | 已登录账号绑手机号 | 目标手机号已属他人 → 返回可合并提示,**不自动合并** | 阶段 2后 |
| POST | `/auth/bind/wechat` | 已登录账号绑微信 | 同上 | 阶段 2后 |
| POST | `/auth/merge/preview` | 合并预览 | 返回将迁移多少条记录、覆盖度会怎么变。**只读,不产生副作用** | 阶段 2后 |
| POST | `/auth/merge/confirm` | 执行合并 | **不可逆**,需带 preview 返回的一次性 token;写 `account_merge_log` | 阶段 2后 |
...
| POST | `/tokens/readonly` | 签发只读令牌 | 前缀 `ro_`,**在 Web 界面手动签发**。scope 一经签发不可提升 | 阶段 2后 |
| GET | `/tokens` / DELETE `/tokens/{id}` | 列出/吊销令牌 | 用 `full` 令牌管理,只读令牌不能管理令牌 | 阶段 2后 |
...
| Method | Path | 用途 | 关键约束 | 阶段 |
|---|---|---|---|---|
| GET | `/billing/plans` | 定价与额度查询 | **匿名可访问。** 档位、价格、额度**由服务端返回,前端不硬编码**——改价、改额度、加档都不发版。**当前只返回两档:`free` 与 `plus`(¥9.9)。** 响应里必须带 `autoRenew: false`(`商业化与额度设计` §6.3) | 阶段 2后 |
| POST | `/billing/orders` | 下单 | body **只接受 `planCode` + `channel`,不接受金额。** 金额由服务端按 `plans` 定;返回 `outTradeNo` + 调起支付所需参数。同一用户同一 `planCode` 存在未支付订单时复用,不新建。**当前 `planCode` 唯一合法值是 `plus`**(`free` 不可下单),但**校验依据是服务端 `plans` 列表,不是写死在代码里的枚举**——重开第二档时这个端点不动 | 阶段 2后 |
| POST | `/billing/notify/wxpay` | 微信支付回调 | 🔴 **不走应用鉴权链路,独立验签。** 按 `transaction_id` 唯一索引幂等——**重复通知返回成功但不重复发放权益**;金额与 `payment_order.amount_fen` 不符 → 拒绝并告警,不发放 | 阶段 2后 |
| GET | `/billing/orders` | 订单列表 | cursor 分页。只返回商品名、金额、状态、时间四类字段 | 阶段 2后 |
| GET | `/billing/orders/{outTradeNo}` | 单笔订单查询 | **客户端支付后必须主动查一次,不能只等回调。** 回调可能延迟或丢失 | 阶段 2后 |
| GET | `/billing/subscription` | 当前会员状态 | 返回档位 + `expiresAt` + `autoRenew: false`(**恒为 false,不是可变字段**) | 阶段 2后 |
| POST | `/billing/orders/{outTradeNo}/invoice` | 开票申请 | 🚧 **只占端点位,不展开。** 发票与税务在 `执行清单` `L-D` 档(阶段 3 后),`商业化与额度设计` §零 已说明本文不重排 | 阶段 3后 |
...
| Method | Path | 用途 | 关键约束 | 阶段 |
|---|---|---|---|---|
| GET | `/quota` | 当前周期额度 | 返回 `ai_capture` / `ai_ask` 两类的 `granted` / `used` / `remaining` + `periodYm`。**只读** | 阶段 2后 |
| POST | `/quota/precheck` | 额度预检 | 🔴 **只读,不扣减。** 剩余为 0 → 返回 `QUOTA_EXHAUSTED`,响应体带手动记录入口提示 | 阶段 2后 |
| — | **额度扣减** | — | 🔴 **没有独立端点。** 见 6.7.1 | — |
...
**最早落地阶段:阶段 2 `1.3.1`(手机号通道)。微信通道在阶段 2 后。**
