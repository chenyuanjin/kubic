...
> 它是「减法」左端的全部内容 —— `盲区 = 骨架层 − 行为层`（定义在 [`后端详设`](../../technical/后端系统设计与组件接入.md) §1.4，本文不改）。
>
> **不是**原料从哪来（在 [`数据线`](../../data/INDEX.md) / [`基础数据`](../../data/基础数据-抓取范围与渠道.md)）、
> **不是**表结构与接口（在 [`技术架构`](../../technical/INDEX.md) §5.2 §6.4）、**不是**排期（在 [`详细排期`](../../execution/阶段0至关卡2详细排期.md) §四）、**不是**界面稿（在设计侧）。
> 本文写到「状态 + 转移条件 + 失败落点」为止，**不定字段、不定接口、不定任何数字**。冲突时一律以被引文档为准。
> **2026-08-31：正文里原本复述过的阈值、题量与接口签名已全部换成指针**（`文档规范` §3.1 那条「不复述」的补齐，裁定人 chenyj）。
>
...
| 步 | 动作 | 产出 | 失败落点 |
|---|---|---|---|
| 1 | 收集近五年公开真题 | 离线原料 | 渠道不可用 → 换渠道，见 `基础数据` 白名单 |
| 2 | 模型初步聚类 → **人工校正分类命名** | 候选 → 已命名 | 工作量失控 → **这本身就是信息**，说明规模化到多模块不可行（`实施路径` §1.2 原话） |
| 3 | 逐题打考点标签 | 每题挂 code | — |
| 4 | 汇总频次统计 | 频次统计齐全（**具体是哪几项在 `技术架构` §6.4，本文不复述**） | 缺任一项 → 不冻结，`E` 出口层的 Top N 排序就没有依据 |

### 2.3 收敛判据与它的失败落点

**种子标注做到「连续 N 题不出现新考点」为止** —— **N 与预计题量在 `详细排期` §C1，本文不复述**。

**不收敛怎么办 —— 这一条必须先写好，因为它发生时人一定在疲劳状态：**

标到 `详细排期` §C1 给的上限仍在出新考点 → **不是继续标，是粒度定错了**（`R-13`）。动作是回到第 2 步重定粒度，**不是把那个 N 往下调**。这是 `实施路径` 那条纪律在本模块的实例：**判据 pass/fail，不是可调目标。**

### 2.4 三层树与分母

`模块 → 题型 → 考点`，**分母只数最底那一层**（层级定义在 `技术架构` §6.4）。中间两层是导航，不是被减数本身 —— 把题型也计进分母，覆盖度会凭空变好看。

### 2.5 一条边界：只做一个模块、一个科目

**两套骨架同时冷启动，对一个人是灾难**（`实施路径` §1.1）。判据是可查的、不是感觉：**有没有为第二个科目做过「人工校正命名」**（`数据线` §六 / `R-12`）—— 做了就是在做第二个模块，不是在积累原料。
...
|---|---|---|---|
| 考点树读取 | 单模块整棵树一次返回，**前端不做懒加载** | 一次返回整棵树并带覆盖度（**签名在真源，本文不复述**） | `技术架构` §6.4 |
| 考点详情 | 展示频次统计 + 我的触达次数 / 最近触达 / 来源集合 | 🔴 **响应里没有讲解字段** | `技术架构` §6.4 · `R-05` |
| 归档节点 | 归档区常驻可见、可找回；归档计数在状态栏 | 归档节点同时退出分子与分母 | `后端详设` §1.4 · `R-49` |
...
| 🔴 **不存题干与机构内容** | 真题原文只在离线区，**线上库没有能装下题干的字段**。考点标签自行命名，不沿用机构体系与措辞 | `数据线` §二 · `R-01` `R-06` |
| ⛔ **不判断对不对** | 考点详情里没有讲解、没有难度分、没有推荐 | `R-05` |
| **归档不做成开关** | 归档是**唯一一个不用真学就能让覆盖率上升的操作** —— 把空白全归档，覆盖率立刻变满。所以归档计数常驻、导出带完整归档清单，三条都不做成可关闭项 | `R-49` |
| **只做一个科目** | 判据是「有没有为第二个科目做过人工校正命名」 | `数据线` §六 · `R-12` |

---

## 六、服务哪个关卡 · 缺口登记

**服务关卡 1。** `总路线图` 编号 `1.2.1`–`1.2.3` `1.2.7`；归档一条挂 `R-49`。

关卡 1 的判据（`实施路径` §关卡1）：**差集结果符合你的自我认知，误判可接受**。频繁把学过的判成「未触达」→ 打标准确率或采集完整度不够 → 回阶段 0 重想。**本模块是那个判据成立的前提，但它自己不产生判据数据。**
...
| # | 缺什么 | 形状 | 谁定 |
|---|---|---|---|
| A-1 | **冷启动工作台的产品形态未定** | 前端无、后端无、流程有 —— 与 `模块地图` §七 那三个缺口形状不同：那三个是「前端有行为、后端无契约」，这个是两边都空 | 产品（我）；但**关卡 1 之前不该定** —— 现在定它就是 `思考模式` §盲区二 |
| A-2 | **频次统计的更新周期未定** | 真题每年新增，统计什么时候重算、重算后覆盖度怎么变，全文档集无处定义 | 产品 + 技术；**登记，不裁定** |

**两条都不在本轮解。** 按 `实施路径`/`总路线图` 只详化到下一个关卡的纪律，关卡 0 未过之前展开它们，就是在把注意力从那两个每日数字上挪走。
