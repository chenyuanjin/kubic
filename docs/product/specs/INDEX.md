...
>
> **不是**模块的状态机与产品判断(在 [`模块地图`](../modules/INDEX.md) 下钻的能力单元)、
...
**十四屏,一屏不多。** ✅ **2026-09-03 产品裁定:`S-ASK` 是第十四屏**,逐屏规格 [问一下](问一下.md),模块层 `U3.7`,端矩阵 `U6.2` §2.2 —— 缺口 `M3-G8` / `M6-G9` 就此闭合。✅ **2026-09-03 同日扫齐:`specs/` 与 `modules/` 两侧 34 处屏数断言一次改完**,`无障碍与多端适配` §二 端矩阵已补 `S-ASK` 行(取值照真源 `U6.2` §2.2)、`端到端验收用例集` §5.1 已按十四屏改并把 `S-ASK` 标 ❌。⚠️ **只剩两格没落,都是明着欠的**:`无障碍与多端适配` §8.2 逐屏焦点链、`S-ASK` 的跨屏验收用例 —— 属于用户 2026-09-02 明确搁置的加厚范围,登记在 [问一下](问一下.md) §四 `Q-2` `Q-4`。🔴 **§5.1 的结论不许改回「全部覆盖」**:那一轮只动了分母。 屏幕编号 `S-XXX` 是设计与开发共用的锚,**改名可以,改编号要同时改本表与各规格文档**。
...
🔴 **下列词一个都不许出现在任何界面、Toast、空态、错误提示、推送、导出文件里**(真源 [`项目现状与决策记录`](../../decisions/INDEX.md) §2、`产品总纲` §四):
...
> ⚠️ **码名的唯一真源是 [`接口契约：签名与错误码全集`](../../technical/接口契约-签名与错误码全集.md) §十**（2026-09-03 技术侧补齐）。
> **本表定的是「每个码长成什么样子」，不是「有哪些码」。**
> ✅ **2026-09-03 产品裁定：下表四行的码名已就地改齐为服务端实际返回的值。**
> 原值 `SMS_CODE_WRONG` / `SMS_CODE_EXPIRED` / `SMS_LOCKED` / `PHONE_TAKEN` 与代码对不上
> (`AuthController.java:380 / :384 / :148,396 / :411`)，照原表实现会让这四种处境**一律落到「未知错误」行** ——
> 四种处境合成一句「没成，再试一次」，正好把这张表存在的理由抵消掉。
> **这里不适用「只加注不改行」**：那条纪律护的是决策层不被执行层改写，而码名不是产品决策，
> 是运行时事实；本表的产品决策是**每个码长成什么样子**，那一列一个字没动。
> 🔴 取右列而不是改代码：运行时是事实，改文档一处比改代码 + 测试 + 文档三处便宜。
...
| `CODE_WRONG` | 页内提示 | 「验证码不对,还能试 {n} 次」 | 重填 |
| `CODE_EXPIRED` | 页内提示 | 「验证码过期了,重新发一条」 | 点重发 |
| `PHONE_LOCKED` | 受限态 | 「错太多次了,{n} 分钟后再试」 | 等 / 换登录方式 |
| `BIND_REFUSED` | 浮层 | 见 [登录与账号](登录与账号.md) §五 | 选合并或换号 |
...
| [额度与购买](额度与购买.md) | `S-BILL` | 可开发 · 依赖支付资质 |
