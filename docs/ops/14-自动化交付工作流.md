...
| 决策层文档 `docs/01`–`04` 的改动 | `ALLOW_DECISION_EDIT=1` | **提交信息自动加 `Decision-layer-edit:` trailer,进历史** |

**出路是刻意留的** —— `R-67` 记过同一件事:**没有出路的守卫,会被撞上它的人在半夜关掉**。但用掉出路必须留痕,所以决策层那条会写进提交历史。

> **自检口径:每月数一次 `git log --grep='Decision-layer-edit'`。** 数字持续增长 = 决策层正在被执行层慢慢改写,而这正是 `05` 那句「04 的关卡判据在这里一个字都不改」要防的事。
...
⚠️ 一个已经修掉、但值得记住的坑:`git diff --cached --name-only` 默认开着 `core.quotepath`,中文路径会被转义成 `"docs/01-\346..."`。**本仓库的文档全是中文名 —— 不关掉这个开关,决策层那道闸会静默失效,而且看起来一直在正常放行。** hook 里已写死 `-c core.quotepath=false`。
