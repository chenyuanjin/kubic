...
| [`U3.8` 我要考的那一场](U3.8-我要考的那一场.md) | 备考档案两个字段：考哪一场、什么时候考；登录成功后那一屏收一次（可跳过）、`S-SET` 可改 | 让差值**贴住用户那一场考试**。⚠️ 今天只服务 `U3.7` 的上下文，**不改排序** |
...
| # | 缺口 | 该谁定 | 展开在 |
|---|---|---|---|
| 🔴 `M3-G1` | 备考档案收集屏（登录成功之后那一屏，**不在 `S-LOGIN` 上**）可跳过，但跳过也占一次点击 —— 「登录之后 2 次点击落下第一条记录」今天读作 3 | **产品**，改齐上游 | `U3.8` §五 |
| ✅ `M3-G2` | ~~「其他考试」这一档是界面推出来的映射~~ **已作废**：产品裁定档案照存全闭集 | —— | `U3.8` §五 |
| ⏳ `M3-G2b` | 契约 `:1941` 的 `EXAM_TYPE_UNSUPPORTED` 没写落在哪个端点；产品已裁「只落统计路径，`PUT` 照存」 | 技术侧改齐 | `U3.8` §五 |
| ⏳ `M3-G10` | 契约 `:1568` 的 `S-BLIND` 上下文里还有 `province?`；产品已裁「删掉，只带三个锚点」 | 技术侧改齐 | `U3.7` §五 |
