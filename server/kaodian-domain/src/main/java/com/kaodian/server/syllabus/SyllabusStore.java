 ...
 * <h2>为什么现在需要它</h2>
 *
 * docs/实施路径 §1.2 的阶段 1 是「骨架冷启动 + <b>人工校正命名</b>」。校正命名的意思是:
 ...
 * 而 R-07 / docs/实施路径 §1.2 要求<b>标签自行命名、不沿用机构既有体系与措辞</b>。
...
public interface SyllabusStore extends SyllabusSource {
     ...
     * (等于把某一种措辞编码进主键)。两条都不走。
     *
     * <p>🔴 <b>父级只能是题型</b>,不能是另一个考点 —— 三层就是三层(决策记录 §2.5)。
     *
     ...
     * (docs/实施路径 §1.2),如果记录挂在名字上,<b>每改一次名就断一次历史</b> ——
     ...
     * 改近五年频次。
     *
     * <p>这个数是<b>统计事实</b>(docs/数据线),也是盲区排序的权重之一
