...
> **流水线本身(分几段、每段进出什么、谁按闸)在 [`骨架数据采集流水线`](骨架数据采集流水线.md)** —— 它是一条纯后端线,没有任何用户界面。
...
**边界只有一句话:数据线可以为四个科目积累原料,但产品先只做一个模块、一个科目。**
...
**这一轮（KUBI-76）只做用户侧。数据采集这棵树本轮不设计** —— 不改流水线、不改抓取范围、不排期。
本节只做两件事：收编骨架层，和把「用户侧可以引用这里的什么」写死。

### 9.1 骨架层收编

[`骨架层`](骨架层.md) 归这条线。判据是「谁看得见」：**它没有终端用户界面**，
交互全部发生在维护者一侧的离线标注工作台，用户在产品里只看到它的结果（考点树），看不到它的过程。

### 9.2 🔴 跨树引用契约：用户侧能引用这里的什么

**用户侧（`docs/product/`）的模块层可以引用这里的口径定义，不许引用采集实现。**

| 允许引用 | 不许引用 |
|---|---|
| 三层树结构（模块 → 题型 → 考点） | 抓取渠道与白名单 |
| **分母只数 level 3 节点** | 离线标注工作台的形态 |
| **归档节点同时退分子与分母，且永远可找回** | 冷启动四步与收敛判据 |
| 频次四统计有哪四个（不含它们的更新周期，那还没定） | 原料存放与隔离方式 |

**为什么要这条例外**：骨架层是减法的被减数，`M2 打标` 与 `M3 盲区与覆盖度` 必然要引用它的口径，
不给例外这两个域根本写不成。**为什么只到口径为止**：再往下就是采集实现，
那部分变化的时钟是风险边界，与产品线的阶段时钟不同 —— 两个时钟等于没有时钟。

### 9.3 运营侧的落点

骨架审核闸「谁来按」这个问题不在这条线里解决，它在 [`运营后台 · 占位`](../ops-console/INDEX.md) `OPS-3` 登记着。
