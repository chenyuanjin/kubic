...
| 3 | **一份文档承载 3–6 个决策** | 超过 6 个就地再拆，不许往下压。**触发拆分的是决策数，不是行数** |

**行数只是判据 3 的代理指标，两个参考值，都不是闸门：**

| 文档里有哪几节 | 参考行数 |
|---|---|
| 只有 §一–§五（流程与意图） | 80–250 |
| 补齐 §六 线框 + §七 交互规格之后 | **≤ 330** |

⚠️ **2026-09-02 从「超 250 行就地再拆」改成上面这样，理由必须写清，否则下一个人会以为这是放水。**
判据 3 的实质是「一份文档承载 3–6 个决策」，行数是它的代理。而补齐 §六 §七 这一波
**没有增加任何一个决策** —— §六 是既有决策的线框呈现，§七 是既有决策的交互展开。
**决策数没变而行数普涨（51 份中位 283、最大 321、47 份越过 250），是代理指标失效，不是文档变胖。**

🔴 **同一次改动里记下这条教训:上一波的闸门报的是「单份超 320 行 0，最长 286」——
320 这个阈值当时没有写在任何一份约定里，286 也不是事实(实测最长 321，且有四份正好停在 320)。
闸门的数字必须能从写下来的约定里读出来。** 读不出来时正确的动作是回来改约定并说明理由，
不是就地换一个数 —— 后者会让闸门变成给已经发生的事背书。

⚠️ **规则 4 是 2026-09-03 补的，因为已经踩了：** `G-1`–`G-15` 这套裸编号在 `M1` `M7` `M8` 三份域台账里**各占一套**，
同一个 `G-4` 在三处分别是「删除后覆盖度怎么回退」「Apple 内购的收据校验」「要不要向既有用户重新征询同意」。
**存量不改名**（改一次要动几百处引用，收益不抵风险），改的是读法与写法两条：

| | 规则 |
|---|---|
| **读** | 不带域前缀的 `G-n` **只在它所在的那一个域里有效**。跨域引用它必须写成「`M7` 的 `G-4`」，裸写 `G-4` 视为无效引用 |
| **写** | 新缺口一律 `M<n>-G<k>`。`M0-G1` `M0-G2` `M8-G1`–`M8-G3` 已经是这个形状，照着走 |

按这三条切下来：**9 个域、53 个能力单元**（2026-09-03 `M3` 增两份，见 §三）。域只做索引，**设计写在能力单元里**。

⚠️ **53 份里有一份不满足判据 1，而且是有意的**：`U7.7` 退款是**缺口登记**，不是已裁定的决策链。
...
| 4 | 🔴 **缺口编号跨域必须带域前缀。** 新开的缺口一律写成 `M<n>-G<k>`（如 `M0-G2` `M8-G1`）| 单元或域索引里出现不带域前缀的新缺口编号 = 违规 |
...
| [**M3 盲区与覆盖度**](M3-盲区与覆盖度/INDEX.md) | `J5` | **减法本身** | 8 | `看盲区`；⚠️ `U3.7` `U3.8` 无对应规格（`M3-G3`） |
...
| `U3.7` 问一下 | `S-ASK` 对话屏 | 🔴 **不是通用聊天框**：四个入口一律带上下文、开屏主体是三条建议问法、回答里的考点可点回 `S-NODE`；额度**按会话扣不按轮扣** |
| `U3.8` 我要考的那一场 | 备考档案两字段 | 首启收一次可跳过、`S-SET` 可改；🔴 **只显示日期不显示倒计时**；⚠️ 今天不改盲区排序 |
...
| `TS-06` | 待补 | 「稍后再认」 | 否 |
| `TS-07` | 手动挂载 | 「已对上」 | ✅ **是** |
| `TS-08` | 确认待补传 | 「已对上 · 等联网」 | **否，直到补传成功** ⚠️ |
| `TS-09` | 挂载失效 | 「考点已归档」 | 否 |

**三条不许违反的约定**：
1. `TS-03` 与 `TS-07` 徽标完全一样 —— **来源不是状态**，用户不需要知道是谁挂的
2. 状态名在所有尺寸形态里**逐字相同**，不做「未匹配」这类同义变体
3. 任何**屏上**形态都**不显示置信度**。空间够不是显示它的理由 —— 显示了就是请用户替产品做判断，而用户没有判断依据。
   ⚠️ **这一条管的是渲染，不是数据留存**（2026-09-03 改，原文写成「任何形态」，比真源更绝对）：
   真源 `打标与未分类` §13.2 的原话是「响应里可以有（服务端阈值裁决要用），但**前端不读、不存、不传给任何渲染层**」，
   `导出与问AI` 的导出字段表里 `tag_confidence` 是一格正式字段。**屏上禁、文件里留**是同一条规则的两面：
   屏上给数字是请用户替产品判断；导出文件是用户把自己的数据带走，抽掉系统握有的字段才是失信。
...
| M2 | `S-TAG` + 卡片内三种尺寸形态 |
| M3 | `S-BLIND` `S-NODE` + 骨架树 + `S-ASK`（`U3.7`）+ `S-SET` 上的一格与首启那一次（`U3.8`） |
| M4 | `S-EXP` + `S-NODE` 上的单考点带走 |
| M5 | `S-LOGIN` `S-ACC` `S-MERGE` |
| M6 | 全部十四屏 × 四端（`S-ASK` 于 2026-09-03 进矩阵，`U6.2` §2.2） |
| M7 | `S-BILL` + 各屏受限态 |
| M8 | `S-SET` `S-RAW` |
