...
- **共同禁令**：任何权限状态下入口都不禁用；「去系统设置」永远排在最后一个（`M0-G2`，2026-09-02），它前面是这一格全部不需要权限的路。
...
| ✅ `M3-G1` 关联 | ~~登录成功之后新增一屏备考档案收集，与本域「登录之后 2 次点击落下第一条记录」相撞~~ | **已闭合，本域一个字不改**：产品 2026-09-03 把收集时点挪到**第一次进 `S-ASK`**，那一屏离开了首启与记录路径，`首次使用与权限申请` §1.2 的 2 与 0 原样保留 | —— |
