...
multica issue metadata set <id> --key repro_cmd --value '"./server/build.sh -q test -Dtest=ApiContractTest"'
```
...
### 9.8 一条还没解决的事:38 条 `in_review` 的存量

规范是从今天起生效的,**存量不会自己消失**。那 38 条是在协议落地之前跑出来的,metadata 全空,按 9.5 的第六行,它们全部落进「无 `repro_cmd` → 原样打回」。

**建议一次性处理,而不是让它们混进新流程:** 挑出与阶段 0 无关的(合规轨 `2.3.x`、数据轨 `3.3.x` 等关卡 0 之后才启动的),直接改回 `backlog`;真正属于阶段 0 的那几条 `issue rerun` 一次,让它们按新协议重跑。

> 这件事本身是**人的活,不要交给分诊器** —— 分诊器读不到 metadata 就只会把 38 条全部打回,而其中大半根本不该在 `in_review` 里。
