 ...
 * <h2>为什么它是一个接口</h2>
 *
 * 与 {@code recognize} 包的 {@code AsrClient} 完全同构,理由也一样(docs/识别链路 坑三):
 * <b>供应商是会变的,切换点必须只有一个。</b>
 * 短信这一侧尤其如此 —— 签名与模板报备各需 1-3 个工作日、要主体资质(docs/技术架构 §7.3),
 * 一旦某家审不过,能换的前提是当初只有一处调用。
 *
 * <h2>🔴 阶段 0/1 的默认实现不发短信</h2>
 *
 * {@link LoggingSmsSender} 把验证码打进日志。这不是「先凑合」——
 * docs/技术架构 §七 写明手机号通道最早落地在<b>阶段 2</b>,而在那之前每发一条真短信
