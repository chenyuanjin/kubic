...
/**
 * 验证码的发与验 —— docs/technical/后端系统设计与组件接入.md §1.8 那两张图的全部实现。
 *
 * <h2>🔴 五道闸的顺序就是这个类的全部要点 —— 是五道,不是四道</h2>
 *
 * <pre>
 *   ⓪ 号码锁定 PhoneLock      → 锁着:拒绝,<b>零外部调用</b>
 ...
 * </pre>
 *
 * 契约(docs/technical/INDEX.md §6.1)把这些约束都列全了,<b>但没有写它们的先后</b> ——
 * 而顺序恰恰是这件事的全部。把①挪到④后面,前面几道闸就只是在给账单排队。
 *
 * <h2>⓪ 为什么排在滑块前面</h2>
 *
 * 排序判据只有一条:<b>这一道拦下来省了多少钱,它自己又花多少</b>
 * ({@code M5-账号与登录通道} §2.1)。⓪ 拦一次省下<b>一条短信 + 一次滑块调用</b>,
 * 而它自己只花<b>一次本地读</b>。
 * <p>
 * 把它挪到滑块后面的代价很具体:一个被锁定的号,每次重发都会白白消耗一次滑块厂商调用 ——
 * 而滑块调用同样有额度。
 * <p>
 * ⚠️ 上一版这里写的是「四道闸」,而代码从 {@link #send} 里那句 {@code store.lockOf}
 * 起一直是五道。这不是文档漏写、也不是代码写多了 —— <b>是那张图少了一格,代码是对的</b>
 * ({@code M5} §十二 登记的两处「文档与实现不一致」之一,改的是注释不是行为)。
 *
 * <h2>四种终态,四句不同的话</h2>
 *
...
public class SmsCodeService {
    ...
    /** docs/technical/INDEX.md §6.1:5 分钟有效。 */
    ...
    public VerifyOutcome verify(String rawPhone, String code, SmsPurpose purpose) {
        ...
        // 而他手机里其实躺着一条能用的新码(docs/technical/后端系统设计与组件接入.md §1.8)。
     ...
     *
     * <p>docs/technical/后端系统设计与组件接入.md §1.8 的那张表列了合并成一句话的后果。用 sealed interface 表达,
