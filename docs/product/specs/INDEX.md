...
|---|---|---|---|---|
| `S-HOME` 首页 | [记一次学习](记一次学习.md) | 不需要 | 不需要 | **不需要** |
| `S-REC` 记一次 | [记一次学习](记一次学习.md) | 不需要 | 不需要 | **不需要**(AI 识别需要) |
| `S-TL` 时间线 | [记一次学习](记一次学习.md) | 不需要 | 不需要 | 不需要(本机)/需要(云端) |
| `S-BLIND` 盲区总览 | [看盲区](看盲区.md) | 不需要 | **需要** | 需要 |
| `S-NODE` 考点详情 | [看盲区](看盲区.md) | 不需要 | **需要** | 需要 |
| `S-TAG` 待补与手动挂载 | [打标与未分类](打标与未分类.md) | 不需要 | **需要** | 需要 |
| `S-EXP` 导出与问 AI | [导出与问AI](导出与问AI.md) | 导出不需要 · 代发需要 | 需要 | 需要 |
...
| `S-SET` 设置 | [设置与原图](设置与原图.md) | 不需要 | 不需要 | 不需要 |
| `S-RAW` 原图管理 | [设置与原图](设置与原图.md) | 不需要 | 不需要 | **不需要** |
...
| **受限** | 登录/额度/骨架缺失 | 说清缺什么 + 去补的入口 + **手动兜底入口** | 不允许把用户堵死在弹窗里 |
...
**未登录不是降级,是阶段 0/1 的正常形态**(`实施路径` §阶段 0 要的是自己用两周)。

| 能力 | 未登录 · 本机 | 已登录 · 免费 | 已登录 · 付费 |
|---|---|---|---|
| 手动记一条 | ✅ | ✅ | ✅ |
| 语音 / 拍照记一条 | ✅ 但**识别要额度** | ✅ 额度内 | ✅ 额度内 |
| 看时间线 | ✅ 本机数据 | ✅ 全部 | ✅ |
| 看盲区 | ✅ 只要骨架能拉到 | ✅ | ✅ |
| 导出 | ✅ 本机数据 | ✅ **不限次数、无水印** | ✅ |
| AI 代发 | ❌ | ✅ 额度内 | ✅ 额度内 |
