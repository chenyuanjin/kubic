 ...
 *
 * docs/09 坑三:选型推荐的视觉模型标注为<b>实验版本,官方未给出任何稳定性承诺、
 * 下线时间表或迁移说明</b>({@code R-35})。缓解手段就写在那条里:识别调用必须走接口层抽象,
 * <b>保留一个切换点</b>。ASR 同理 —— 首选腾讯云一句话识别(免费额度 5,000 次/月,
 * 覆盖到关卡 3),但换成本地 whisper 或别家,应当只是换一个实现类。
 * <p>
 * docs/10 §2.1 把这一层画成整个后端<b>唯一允许出现外部模型调用的地方</b>。
 * 别处出现 HTTP 调模型,就是绕开了这个切换点。
 *
 * <h2>🔴 签名上没有 URL、没有 fileId、没有任何存储路径</h2>
 *
 * 入参是音频<b>字节</b>,一次过、用完即弃。docs/10 §八:服务端不写磁盘、不进对象存储。
 ...
 * {@link com.kaodian.server.collect.Touch} 里没有能装下它的字段(01 §2.2 不碰内容),
 * 这不是「不填」,是结构上没有这个位置。
 *
 * <h2>当前还没有调用点,这是有意的</h2>
 *
 * 「文字 → 考点」的闭集匹配属于打标管线({@code 08 §1.2.5}),那一段还没建。
 * docs/10 §3.1 的原话是「<b>这两个接口的签名是本文档里唯一值得现在就写下来的代码</b>」——
...
public interface AsrClient {
     ...
     * @throws RecognitionUnavailableException 服务不可用/未配置。<b>失败就抛,不返回半成品</b>(docs/10 §3.1)
     */
