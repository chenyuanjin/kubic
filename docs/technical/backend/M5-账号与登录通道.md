...
### 增量 10 · 🔴 `WeChatEntry` 的三个 wireName 与契约对不上 —— 实现期发现,改的是代码

> **实读 `KUBI-104` 基线(`origin/v1` @ `000d403`),`WeChatEntry` 的 wireName 是 `mini_program` / `official_h5` / `website_qr`,而 §9.3 定的契约值是 `mini` / `official_h5` / `open_web`。三个里对上一个。**
>
> **裁定:改代码,按契约走。** 这一处上一版没被登记进 §十二 的活冲突表,因为它藏在枚举的 `wireName` 里而不是在端点签名上 —— **而 §3.6 判据 ① 恰好会撞红它**(`grep 'mini\|official_h5\|open_web'` 在老代码上只有 2 个命中,不是 3 个)。
>
> ⚠️ **这是一次真实的 wire 格式变更**,但代价为零:`grep -rn 'mini_program\|website_qr' web/src` 零命中 —— 微信整条通道在阶段 2 后,今天没有任何一个端在传这个字段。**能零代价改的时点就是现在**,等有端在传就要两边同批发。

### 增量 11 · §9.3 `state` 那一行,把「`mini` 必须不带」落成一个显式失败

> | `state` × `mini` | 🔴 **带了就 `400 WECHAT_STATE_INVALID`,不是静默忽略。** 静默忽略等于让「三条入口是一条」这个误解一直活着 —— 而它下一次现身是在 `official_h5` 上漏传 `state`,那一次是真的 CSRF 面。落点:`AuthController#exchange` 里一个显式的 `entry == MINI_PROGRAM` 分支,**不是一个 `state == null` 判断**(§3.2:后者会在某次重构里悄悄扩大到别的入口)。 |

### 增量 12 · §1.1 标识那一行,补一条发号器的连带约束

> | 账号 id 的连续性 | 🔴 **发号器只保证「不重复」,不保证「不跳号」。** 建号撞上身份冲突会烧掉一个号,重启也会把「发出去但没用掉」的号重新发一次(那是安全的:没建成账号,那个号在任何地方都没有引用)。
> **要求不跳号的代价:** 失败时得把号还回去,而那是一个会写错的回滚。**「id 是连续的」不许被任何一处代码当成前提** —— 尤其不许拿 `max(id) - 10001` 当注册数,那个数归 `SignupLedger`。 |
>
> ⚠️ **这一条是实现期被并发测试逼出来的,不是设计时想到的。** 老的 `u_`+UUID 天然躲开了它(随机不用读当前最大值);换成连续 id 之后,「读最大值 + 1」会让登录页连点两次撞出一个 `500`(`AccountStore.create` 抛的 `IllegalStateException` 不在 `createOrJoin` 的捕获范围里)。判据:`UserIdTest#neverIssuesTheSameIdTwice` 与 `ConcurrencyTest`。

### 增量 13 · §7.4 `GET /account` 那一行,删 `nickname`

> | `GET /account` | 响应里**没有 `nickname`**(§9.9)。⚠️ **前端同批改**:`web/src/api/auth.ts` 的 `AccountDto` 上那一行一起删 —— 与 §9.1 `devCode` 那条同一形态,**契约赢不等于可以不看调用方**。本次零代价:`grep -rn 'nickname' web/src` 只有类型声明一处,没有任何一处在读它。 |
