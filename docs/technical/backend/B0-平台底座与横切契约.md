...
| `B0-6` | 分页与幂等 | 分页只有游标一种,形状 `{items, nextCursor?}`;幂等三种键各有其名。**请求键的端点全集以 `接口契约` §3.3 八行为唯一真源**,其中**不含退款** | DTO 里 `total`/`hasMore`/`page`/`offset` 零命中 —— **今天不是零**(§七) |
...
| **业务实体主键** | 🆕 **同样 `long`,JSON 里同样是字符串。** `Touch.id` / `RecordTag.id` / `TagAttempt.recordId` / `jobId` / `nodeId` 全部 `long`。⚠️ **例外只有一个:只读令牌的 `tokenId` 是不透明串**(`M5` §十三 增量 4 已登记),因为它同时是给用户看的东西 |
...
| `/billing/notify/wxpay` | 🔴 **不是匿名,是另一条鉴权链** —— 独立验签,过滤器放行后由该 controller 自己验签。放进这张表是为了让「匿名的只有六个」不被误读成「回调漏了」 |
...
🔴 **要用 `Idempotency-Key` 的端点全集,见 `接口契约` §3.3 那张八行表 —— 那是唯一真源。**
...
| 2 | **`401` 拆不拆档** | `接口契约` §1.2:必须区分 `UNAUTHORIZED` / `TOKEN_EXPIRED`;`CurrentSessionResolver.java` 注释:四种失败对外同一个 401,「对攻击者却是信息」 | **契约赢,拆三档**(§5.3)。⚠️ **2026-09-03 更正(KUBI-89 审核轮):本行上一版写「两档」。** 「已吊销」本身仍不单独成档,解析器那条顾虑原样保住 —— 第三档 `ACCOUNT_DEACTIVATED` 的依据是**账号状态不是令牌状态** |
...
四份模块设计各数出一个数(`B0` 三 / `M2` 四 / `M4` 五 / `M7` 五),stage 3 已取**并集八行**并落进 `接口契约` §3.3。
🔴 **照上一版合并会把那张八行表改回三行,直接丢掉五个端点的幂等要求** —— 下面是终稿,行数断言已删。

> **这张表补第四列 `幂等`,取值 `—` / `Idempotency-Key`。** **要标 `Idempotency-Key` 的端点全集见 §3.3 那张表 —— 那是唯一真源,本列逐行照它标,不在别处另数一遍。**
> `B0` 自己认领其中三行:`POST /billing/orders`(外部账单)、`POST /ai/ask`(🔴 外部模型调用 = 按次外部账单,重放一次扣两次额度)、`DELETE /account`(不可逆改变账号状态);其余五行分别归 `M2` / `M4` / `M7`。
>
> §1.5 写着「哪些端点必须带 `Idempotency-Key`,在 §三 的总表里逐行标 `幂等` 列」,而那一列此前不存在 —— 一条指向空处的指针,读的人会以为自己看漏了。

### 增量 3 · §8.8 退款那一节,补一句幂等的话

> ⚠️ **顺带关掉一处会被反复问起的联想:退款既然一个端点都不补,它就没有幂等语义。** §1.5 的三种幂等键里,「请求键」的端点全集(`接口契约` §3.3 那八行)**不含退款**,一行都没有。
...
| KUBI-93 | `M4` 出口与令牌 | `B0-4` 只读令牌四道锁 + `B0-6` 幂等(`POST /ai/ask` 是 `接口契约` §3.3 八行之一) |
...
🔴 **第 2 条有一个必须点破的连带后果:本文的白名单常量按 `/api/v1` 写,而代码里十个 controller 全部挂在 `/api/**` 下。** 迁移前那七行匹配不到任何东西 —— **这是本文唯一一处「写了但今天不生效」的东西**。`B0-4` 落地时二选一:要么同时迁前缀,要么白名单临时写 `/api`。**不许两个前缀都写一遍** —— 那会让「匿名入口的全集在这一处数得清」当场失效,而那正是白名单存在的全部理由。
