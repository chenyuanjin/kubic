...
| 2 | **豁免的依据必须在被引的文档里找得到;引不出原文的豁免即刻失效** | 一条「依据在别处」的豁免,等于**没有依据的豁免**。在那句判据被真正写进 `docs/execution/INDEX.md` 之前,以 `design/README.md:45` 的平铺禁令为准 |
...
⚠️ **还有一条必须由人裁、技术侧不自行放宽也不自行收窄:**
「用户自己填的两个整数相除得到的正确率」到底算不算 `design/README.md:45` 禁的那个「正确率」?
`design/README.md` 与那个脚本对同一件事给了相反答案,而**两边都是项目级规则**。
这是产品边界不是技术选型 —— 已升给 chenyj,在它落定之前按第 2 条走严的那一侧。
