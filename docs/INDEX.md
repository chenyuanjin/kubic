...
> [`文档规范与目录` §3.2](文档规范与目录.md)。**目录只是收纳，不是身份** —— 跨文档引用一律写文档标题 + 链接（`` 见 [`后端系统设计与组件接入`](technical/后端系统设计与组件接入.md) §6.9 ``），不另起简称。
>
...
> 规则与理由在 [`文档规范与目录` §3.2.1](文档规范与目录.md)，旧号 → 现在的文件对照表在 §2.3。
...
`misc/` 按定义没有主文档；`modules/` 的唯一清单在 [`产品模块地图与产品逻辑`](product/INDEX.md) §三/§九，
在这一层再放一份就是第二处说法（`文档规范与目录` §3.1）。**子目录不自动配 `INDEX.md`，只有主文档存在时才配。**
...
唯一真源仍是 [`文档规范与目录` §2.1](文档规范与目录.md) 那张表 —— 目录第一级只能承载一个维度，
...
**这里数的是活跃文档**；磁盘上是 31 个文件 / 12,015 行（另有 `research/` 两份原件 1,047 行，不进这个口径），多出来的那一份是作废但按 `文档规范与目录` §4.3 不删的 `产品路线图与用户共创`。份数与行数的唯一真源仍在 [`文档规范与目录` §二](文档规范与目录.md)。

| 你要干什么 | 路线 | 行数 |
|---|---|---|
| **搞懂这是什么** | [`项目现状与决策记录`](decisions/INDEX.md) → [`产品模块脑图`](product/产品模块脑图.md) → [`产品模块地图与产品逻辑`](product/INDEX.md) → [`实施路径`](decisions/实施路径.md) | ~940 |
| **搞懂某个模块怎么运转** | [`产品模块脑图`](product/产品模块脑图.md) 找到它挂在哪一组 → 该组的展开文档 [`模块 A · 骨架层`](product/modules/模块A-骨架层.md)–[`模块 G · 商业化`](product/modules/模块G-商业化.md)（A–G 各一份）。**H 组当前没有展开文档**（原来那份 `产品路线图与用户共创` 已作废），**X 不单独成文** —— 逐组落点见 `产品模块地图与产品逻辑` §九 | ~185/份 |
...
`产品模块地图与产品逻辑` 与 `产品模块脑图` 是同一格（地图 + 脑图）。不读这几份，后面每一份都会读歪。
...
| [`产品模块地图与产品逻辑`](product/INDEX.md) + [`产品模块脑图`](product/产品模块脑图.md) | 产品由哪些模块组成、每个模块归哪个阶段。脑图在 `产品模块脑图`（markmap 格式，可折叠；不装工具打开是可读大纲） | 讨论功能之前 |
...
**模块展开文档 `模块 A · 骨架层`–`模块 G · 商业化`**（A 骨架层 / B 采集层 / C 打标层 / D 差集层 / E 出口层 / F 账号与端 / G 商业化，7 份 ~1,310 行）也在这一档：
**按模块读，不按目录顺序读** —— 你在改哪个模块就只读那一份，每份都自带「状态 + 转移条件 + 失败落点 + 交互示意 + 边界」。
其中 [`模块 B · 采集层`](product/modules/模块B-采集层.md) 是唯一整个服务**阶段 0** 的一份，也就是当前唯一有效的工作面。
...
> （`find docs -name '*.md' -not -path '*/research/*' | xargs wc -l`）—— 上一版就是塞进了 `模块 A · 骨架层`–`模块 E · 出口层` 却没改 L1 那个数。
>
...
|---|---|---|
| **整体** | [`项目现状与决策记录`](decisions/INDEX.md) 现状与决策 · [`产品模块地图与产品逻辑`](product/INDEX.md) 模块地图 · [`产品模块脑图`](product/产品模块脑图.md) 脑图 | [`技术架构与接口契约`](technical/INDEX.md) 架构与接口契约 |
| **展开** | [`模块 A · 骨架层`](product/modules/模块A-骨架层.md)–[`模块 G · 商业化`](product/modules/模块G-商业化.md) 模块 A–G 展开 · [`商业化与额度设计`](product/商业化与额度设计.md) 商业化与额度 · ⛔ **H 组无展开文档**（原 [`产品路线图与用户共创`](misc/产品路线图与用户共创-已作废.md) 已作废） | [`后端系统设计与组件接入`](technical/后端系统设计与组件接入.md) 后端详设 · [`多端选型与端矩阵`](technical/多端选型与端矩阵.md) 端矩阵 · [`Agent 框架与能力边界`](misc/Agent框架与能力边界.md) Agent 框架（收纳在 `misc/`） |
...
新增或移动文档：走 [`文档规范与目录` §六](文档规范与目录.md) 的动作清单，**同一次改动里 `文档规范与目录` §二 与本文 §二/§四 都要更新**，
漏一边就是「同一件事有多处说法」的下一个实例。

**模块展开文档（`模块 A · 骨架层`–`模块 G · 商业化`）的特例**：它们只在 `产品模块地图与产品逻辑` §三/§九 与 `产品模块脑图` 里被挂载，**不另建第二张模块清单**。
新增一份模块文档要改三处：`文档规范与目录` §2.1 加行、`产品模块地图与产品逻辑` §三/§九 加指针、本文 §零/§一/§二/§四/§五（全量清单）。**文件落在 `product/modules/`，那一层不设 `INDEX.md`**（清单在 `产品模块地图与产品逻辑`，见 §零）。
**作废一份也一样要走这三处** —— 动作清单在 [`文档规范与目录` §六 第 10 条](文档规范与目录.md)。

**新文档放哪个子目录**：规则在 [`文档规范与目录` §3.2](文档规范与目录.md)，判据是「谁来读」，不是「属于哪条管线」。
