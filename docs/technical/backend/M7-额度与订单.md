...
| 3 | **`B0` 增量 2 的幂等列只写了三行** | `B0` §十五 增量 2：三行；`接口契约` §8.4 / §8.5 示例里逐字写着 `Idempotency-Key` | ⚠️ **2026-09-03 更正**：本文上一版判「五行」，四份各数一个数（`B0` 三 / `M2` 四 / `M4` 五 / `M7` 五），KUBI-96 已取**并集**并落进 `接口契约` §3.3。**该表是唯一真源，本文不再自己数**（§8.1） | 已落地，`接口契约` §3.3 |
...
四份模块设计各数出一个数，KUBI-96 stage 3 已取**并集**并落进 `接口契约` §3.3(⚠️ 那次并集后来又发现漏了 `M5` 一行 —— 所以本文更不该记这个数)。**该表是唯一真源，本文不再复述行数。**

> **本模块认领 `接口契约` §3.3 那张表里的三行**：`POST /billing/orders`（外部账单）、`POST /billing/orders/{outTradeNo}/close`、`POST /billing/orders/{outTradeNo}/receipt/verify`（后两者的示例里本来就逐字写着这个头，见 §8.4 / §8.5）。`POST /ai/ask` 与 `DELETE /account` 同在那张表上，但分别归 `B0`/`M4` 与 `B0`/`M5` 认领。
...
**下面三条是实现轮次（KUBI-101）写代码时发现的，不是设计轮次就知道的。** 三条都已在代码里按「不自己放宽」处置，形状与理由如下。

### 增量 13 · §6.7 `POST /quota/precheck` 的请求体喂不出 `manualEntry` 的第三个取值

> 🔴 `QUOTA_EXHAUSTED` 的 `details.manualEntry` 有**三个**取值（`manual_text` / `manual_tag` / `copy_context`，对应 `U7.2` §2.5 的三种免费兜底），
> 而 `POST /quota/precheck` 的请求体**只有 `quotaType` 一个字段**。
> `ai_capture` 这一个池子**同时盖着语音转写与图片识别打标**（§2.1）—— 前者的兜底是「手动补一句」（`manual_text`），后者是「手动挂考点」（`manual_tag`）。
> **只凭 `quotaType` 分不出是哪一个**，于是 `manual_text` 在这条路上取不到。
>
> **本轮处置**：按 §8.3 那个响应示例逐字取 `ai_capture → manual_tag`、`ai_ask → copy_context`，
> 🔴 **不自己往请求体上加一个字段去补这一格** —— 那是一次契约变更，不是一次实现细节。
> **要补的话补哪一个由 `M2` 决定**：转写与打标的扣点时机归 `M2`（KUBI-102），
> 它知道这一次调用是哪一段，也就知道该不该给 precheck 加一个 `scene` 字段。
> ⚠️ 在它定之前，`manual_text` 这个取值**在服务端不可达** —— 端的词表里可以留着它，但服务端今天不会下发。

### 增量 14 · §8.3 `GET /billing/channels` 的「端标识 ∩ 配置」少了前半截

> §5.2 写的是「服务端按**请求头里的端标识** + 配置里的开通状态取交集」。
> 🔴 **实现轮次实测：整个文档集里没有任何一处定义过那个端标识请求头**（头名、取值域、缺失时怎么办，一处都没有）。
> **本轮只做了后半截**（按配置里的开通状态给），🔴 **没有自己发明一个头** —— 发明一个头就是凭空造一条四个端都要跟着实现的契约。
>
> 于是今天的实际语义是「**这一次部署开了哪些通道**」而不是「**这一端能用哪些通道**」。
> 两者在只有一个端的今天没有区别；**第二个端上线那天有区别**，而那一天要先定这个头。
> 归 stage 3 与 `多端选型与端矩阵` 那一轨一起定，**不由本模块单方面补**。

### 增量 15 · §1.3 错误体：`ApiException` 没有装 `details` 的地方

> 🔴 `接口契约` §1.3 的错误体有 `details`，`ApiError` 那一侧的四参构造器也留着通道（它的类注释逐字点了 `QUOTA_EXHAUSTED` 这一处）——
> **但 `B0` 的 `ApiException` 上没有 `details` 字段，`ApiExceptionHandler` 出的一律是三参 `ApiError`（`details` 恒 `null`）。**
> 也就是说：**通道留了，从异常到通道的那条路没修**。
>
> **本轮处置**：`M7` 在自己的包里另起 `QuotaExhaustedException` + 一个只认它的 `@RestControllerAdvice`（`BillingExceptionHandler`），
> 🔴 **`B0` 的两个文件一个字不动**（KUBI-101 共同约束第 2 条：改包络形状要停手报回）。
> ⚠️ **这不是一个只有 `M7` 会撞上的缺口**：契约今天登记在册的 `details` 有三处，另两处（`PHONE_LOCKED` 带解锁时点、`CODE_WRONG` 带剩余次数）归 `M5`；
> `M2` / `M4` 在扣减耗尽时抛的也是同一个 403。**四条分支各造一个 advice = 四套语义。**
> **正解是把 `details` 补进 `B0` 的 `ApiException`（一个字段 + handler 里一行）**，由 `B0` 的执行人或 stage 3 做；
> 补完之后本模块那个 advice 连同异常类一起删，`throw new ApiException(QUOTA_EXHAUSTED, msg, details)` 即可。
...
| `§契约增量` | §十三（**15 条**，stage 3 串行合并；13-15 是实现轮次 KUBI-101 新加的） |
