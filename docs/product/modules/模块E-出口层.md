...
>
> **屏幕级规格在 [`导出与问AI`](../specs/导出与问AI.md)** —— 那一份写到可以直接做 UI 与前后端开发,本文只写产品判断。
>
...
| 出口 | 形态 | 性质 |
|---|---|---|
| **盲区清单** | 哪些考点一次都没碰过 | 🌟 **北极星落点** |
| **先补这几个 · Top N** | 按频次排序，排除已断言节点 | 北极星的排序层 |
| **问 AI · 免费复制** | 组装上下文，用户自己粘出去 | ⚠️ **契约缺口**（见 §六 E-1） · 🔴 永不移除 |
| **问 AI · AI 代发** | 产品代发并原样透出回答 | ⚠️ **契约缺口** · 付费增强（`产品模块地图与产品逻辑` §三） |
| **导出 md / csv / json** | 全量、无删减 | 🔴 **永不上锁** |
| **MCP / CLI · 只读** | 一组只读 tool（**几个在 `技术架构与接口契约` §6.5**） | 营销资产，不是收入线 |
...
待办树编号：盲区清单与 Top N `1.2.8.4`（`总路线图 · 脑图与父子待办`）。导出 `1.3.6` 🔴。MCP `1.4.2`。
